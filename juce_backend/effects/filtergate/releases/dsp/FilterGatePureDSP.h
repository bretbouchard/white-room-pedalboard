...
// Predefined policies
constexpr FilterGatePolicyConfig FilterGateChannelStripPolicy {
...
constexpr FilterGatePolicyConfig FilterGateFXPolicy {
...
//==============================================================================
// Spectral Curve Types (Frequency Weighting)
//==============================================================================

enum class SpectralCurve
{
    Flat,               // No frequency weighting (1.0 across spectrum)
    LowTilt,            // Lows weighted more (1-x)
    HighTilt,           // Highs weighted more (x)
    ExponentialLow,     // Exponential low bias (pow(1-x, k))
    ExponentialHigh     // Exponential high bias (pow(x, k))
};

//==============================================================================
// Energy Accumulation Modes (Gate Decision Logic)
//==============================================================================

enum class EnergyMode
{
    Independent,        // Per-band gating (original behavior)
    WeightedSum,        // All bands contribute to shared decision
    LowBiasedSum,       // Low frequencies dominate gate decision
    HighBiasedSum       // High frequencies dominate gate decision
};
...
struct SpectralWeight
{
    // Calculate frequency weight for gate decision
    // freqNorm: normalized frequency (0.0 = low, 1.0 = high)
    // curve: spectral bias curve type
    // exponent: power for exponential curves (default 2.0)
    static float calculate(float freqNorm, SpectralCurve curve, float exponent = 2.0f)
    {
        freqNorm = std::max(0.0f, std::min(1.0f, freqNorm));  // Clamp to [0,1]

        switch (curve)
        {
            case SpectralCurve::Flat:
                return 1.0f;

            case SpectralCurve::LowTilt:
                return 1.0f - freqNorm;

            case SpectralCurve::HighTilt:
                return freqNorm;

            case SpectralCurve::ExponentialLow:
                return std::pow(1.0f - freqNorm, exponent);

            case SpectralCurve::ExponentialHigh:
                return std::pow(freqNorm, exponent);
        }
        return 1.0f;
    }
};
...
class FilterGateSmoother
{
public:
    FilterGateSmoother() = default;
...
public:
    FilterGateDSP()
    {
        // Default to FX policy
        setPolicy(FilterGateFXPolicy);
    }
    ...
    void prepare(double sampleRate, int maxBlockSize)
    {
        ...
        freqSmoother_.prepare(sampleRate, 10.0f);
    ...
    void reset()
    {
        ...
        gate.reset();
        freqSmoother_.reset(frequency_);
    ...
    void setPolicy(const FilterGatePolicyConfig& policy)
    {
        policy_ = policy;
    ...
    const FilterGatePolicyConfig& getPolicy() const { return policy_; }
    ...
    void setFrequency(float hz) { freqSmoother_.setTarget(hz); }
    ...
    //==========================================================================
    // Spectral Parameters (NEW)
    //==========================================================================

    // Frequency weighting curve
    void setSpectralCurve(SpectralCurve curve) { spectralCurve_ = curve; }
    void setSpectralExponent(float exp) { spectralExponent_ = std::max(0.1f, std::min(10.0f, exp)); }

    // Energy accumulation mode
    void setEnergyMode(EnergyMode mode) { energyMode_ = mode; }

    // Gate floor (partial openness) - prevents hard on/off
    void setGateFloor(float floor) { gateFloor_ = std::max(0.0f, std::min(0.5f, floor)); }

    // Adjacent band linking (spectral continuity)
    void setBandLinking(float amount) { bandLinking_ = std::max(0.0f, std::min(1.0f, amount)); }
    ...
    void processStereo(float* left, float* right, int numSamples)
    {
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            float frequency = freqSmoother_.processSample();
            ...
            if (gateEnabled_)
            {
                ...
                // Apply gate floor (partial openness) for musical results
                // Instead of 0.0-1.0, we have gateFloor_ to 1.0
                float gatedGain = lerp(dbToGain(-gateRange_), 1.0f, gateAmount);

                // Apply gate floor: minimum openness prevents brittle sound
                float finalGain = lerp(gateFloor_, 1.0f, gateAmount);

                left[i] = filteredLeft * gatedGain * finalGain;
                right[i] = filteredRight * gatedGain * finalGain;
            }
            else
            {
                // Gate bypassed
    ...
    //==========================================================================
    // Multiband Support (Adjacent Band Linking)
    //==========================================================================

    // Apply adjacent band linking for spectral continuity
    // This is designed for multiband implementations where multiple
    // FilterGateDSP instances process different frequency bands.
    //
    // Usage in multiband context:
    //   float bandEnergies[] = { low, midLow, mid, midHigh, high };
    //   for (int i = 0; i < numBands; ++i) {
    //       bandEnergies[i] = applyBandLinking(bandEnergies, i, numBands);
    //   }
    //
    static float applyBandLinking(const float* bandEnergies, int bandIndex, int numBands, float linkAmount)
    {
        if (linkAmount <= 0.0f || numBands <= 1)
            return bandEnergies[bandIndex];

        float linkedEnergy = bandEnergies[bandIndex];
        float weightSum = 1.0f;
        float weightedSum = linkedEnergy;

        // Link with previous band
        if (bandIndex > 0)
        {
            weightedSum += bandEnergies[bandIndex - 1] * 0.5f;
            weightSum += 0.5f;
        }
        ...
        // Blend between original and linked based on linkAmount
        float linkedValue = weightedSum / weightSum;
        return linkedEnergy * (1.0f - linkAmount) + linkedValue * linkAmount;
    }
...
private:
    ...
    void updateControlRate()
    {
        ...
        float frequency = freqSmoother_.getTarget();
    ...
    //==========================================================================
    // Detector Calculation (Spectral-Aware Gate Logic)
    //==========================================================================

    inline float calculateDetector(float left, float right)
    {
        // Get base detector value from trigger mode
        float baseDetector = getBaseDetector(left, right);

        // Apply spectral weighting if enabled
        if (spectralCurve_ != SpectralCurve::Flat)
        {
            // Estimate frequency from current filter frequency
            // Normalize to [0,1] based on audio range (20Hz - 20kHz)
            float freqNorm = std::log(frequency_ / 20.0f) / std::log(1000.0f);  // Log scaling
            freqNorm = std::max(0.0f, std::min(1.0f, freqNorm));

            // Apply spectral weight
            float spectralWeight = SpectralWeight::calculate(freqNorm, spectralCurve_, spectralExponent_);

            // Combine with base detector based on energy mode
            return applyEnergyMode(baseDetector, spectralWeight);
        }

        return baseDetector;
    }

    inline float getBaseDetector(float left, float right)
    {
        switch (triggerMode_)
        {
    ...
    inline float applyEnergyMode(float detector, float spectralWeight)
    {
        switch (energyMode_)
        {
            case EnergyMode::Independent:
                // Per-band behavior (spectral weight doesn't affect decision)
                return detector;

            case EnergyMode::WeightedSum:
                // All bands contribute equally, weighted by spectral curve
                return detector * spectralWeight;

            case EnergyMode::LowBiasedSum:
                // Low frequencies have more influence
                return detector * (1.0f + spectralWeight * 0.5f);

            case EnergyMode::HighBiasedSum:
                // High frequencies have more influence
                return detector * (0.5f + spectralWeight * 1.0f);
        }
        return detector;
    }
    ...
    // Policy
    FilterGatePolicyConfig policy_;
    ...
    FilterGateSmoother freqSmoother_;
    FilterGateSmoother resonanceSmoother_;
    FilterGateSmoother gainSmoother_;
    ...
    // Spectral parameters (NEW)
    SpectralCurve spectralCurve_ = SpectralCurve::Flat;
    float spectralExponent_ = 2.0f;
    EnergyMode energyMode_ = EnergyMode::Independent;
    float gateFloor_ = 0.0f;          // Minimum gate openness (0.0-0.5)
    float bandLinking_ = 0.0f;         // Adjacent band smoothing (0.0-1.0)
