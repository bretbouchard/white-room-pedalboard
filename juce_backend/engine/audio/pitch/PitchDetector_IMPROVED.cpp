...
void PitchDetector::processBlock(juce::AudioBuffer<float>& buffer) {
    ...
    if (result1.first > 0.0 && result1.second > 0.1) candidates.push_back(result1);
    if (result2.first > 0.0 && result2.second > 0.05) candidates.push_back(result2);
    if (result3.first > 0.0 && result3.second > 0.1) candidates.push_back(result3);
    ...
    if (detectedFrequency < 150.0) {
        adaptiveThreshold = 0.15; // Lower threshold for low frequencies
    }
