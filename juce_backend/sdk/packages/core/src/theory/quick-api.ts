...
export async function quickAnalyzeChords(
  ...
  try {
    ...
    const detectedKey = key || keyResult?.data?.tonic?.name + ' ' + (keyResult?.data?.scale?.characteristics?.quality || 'major');
    
    const functionsResult = await engine.analyzeHarmonicFunction(chords, detectedKey);
    const suggestionsResult = await engine.suggestImprovements(chords, detectedKey);
 ...
 * Quick key detection
 */
export async function quickDetectKey(
 ...
 * Quick chord analysis
 */
export async function quickAnalyzeChord(
  chordSymbol: string
): Promise<TheoryEngineResult<{
  name: string;
  root: string;
...
export async function quickSuggestNextChords(
  ...
  try {
    const engine = getTheoryEngine();
    const result = await engine.suggestNextChords(currentChords, key, { count });
    ...
    return createResult<{
      suggestions: Array<{ chord: string; function: string; probability: number; reasoning: string }>;
    }>(false, undefined, {
      code: TheoryEngineError.ANALYSIS_FAILED,
...
export const quickIntervalUtils = {
  ...
  getIntervalQuality(semitones: number): string {
    const qualities = [
      'perfect', 'minor', 'major', 'minor', 'major', 'perfect',
      'augmented', 'perfect', 'minor', 'major', 'minor', 'major'
    ];
...
export const quickChordUtils = {
  ...
  containsNote(chordSymbol: string, noteName: string): boolean {
    ...
    return chordTones.some(tone => 
      tone.replace(/\d+$/, '') === noteWithoutOctave
    );
  },
};
...
export const quickScaleUtils = {
  ...
  isNoteInScale(noteName: string, scaleName: string, tonic: string): boolean {
    ...
    return scaleNotes.some(note => 
      note.replace(/\d+$/, '') === noteWithoutOctave
    );
  },
  ...
  getScaleDegree(noteName: string, scaleName: string, tonic: string): number {
    const scaleNotes = this.getScaleNotes(scaleName, tonic);
    const noteWithoutOctave = noteName.replace(/\d+$/, '');
    const index = scaleNotes.findIndex(note => 
      note.replace(/\d+$/, '') === noteWithoutOctave
    );
   ...
   */
  getRelativeScale(scaleName: string, tonic: string): { scale: string; tonic: string } {
    if (scaleName === 'major') {
      const relativeTonic = quickNoteUtils.transposeNote(tonic + '4', 9).replace(/\d+$/, '');
      return { scale: 'minor', tonic: relativeTonic };
    } else if (scaleName === 'minor') {
      const relativeTonic = quickNoteUtils.transposeNote(tonic + '4', 3).replace(/\d+$/, '');
