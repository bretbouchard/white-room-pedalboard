...
export class MelodyAPI {
  ...
  /**
   * Generate a melodic contour using Schillinger principles
  ...
  async generateContour(
    contour: ContourType,
    length: number,
    options: Partial<MelodyGenerationParams> = {}
  ): Promise<MelodyLine> {
    // Validate inputs
    this.validateContourType(contour);
    this.validateLength(length);

    const params: MelodyGenerationParams = {
      key: options.key || 'C',
   ...
   */
  async generateLine(params: MelodyGenerationParams): Promise<MelodyLine> {
    // Validate inputs
   ...
   */
  private generateContourOffline(params: MelodyGenerationParams): MelodyLine {
    try {
      const {
        key,
        scale,
   ...
   */
  private generateBasicMelodyFallback(params: MelodyGenerationParams): MelodyLine {
   ...
   */
  private generateLineOffline(params: MelodyGenerationParams): MelodyLine {
  ...
  private analyzeMelodyOffline(
    ...
    try {
      ...
      const range = [Math.min(...melody.notes), Math.max(...melody.notes)] as [number, number];
      const tessitura = this.calculateTessitura(melody.notes);

      // Sophisticated phrase detection with musical phrasing principles
      const phrases = options.includePhrases !== false
        ? this.detectMusicalPhrases(melody.notes, intervals)
        : [];

      // Advanced complexity analysis
      const complexity = options.includeComplexity !== false
        ? this.calculateMelodicComplexity(melody.notes, intervals, harmonicImplications)
        : undefined;
      ...
      return {
        ...
        complexity,
        contourSegments: options.includeContour !== false ? contourSegments : [],
        motificAnalysis: this.analyzeMotifs(melody.notes, intervals),
        rhythmicCharacteristics: this.analyzeRhythmicCharacteristics(melody)
      };
    } catch (error) {
      safeLog('MelodyAPI.analyzeMelodyOffline', error, 'warn');
      // Fallback to basic analysis if enhanced analysis fails
      return FALLBACKS.melodyAnalysis;
    }
  }
  ...
  private analyzeHarmonicImplications(intervals: number[]): {
    ...
    const tensionPoints = intervals
      .map((interval, index) => harmonicQualities[index] === 'dissonant' ? index : -1)
  ...
  private determinePrimaryContour(segments: Array<any>): string {
    ...
    const ascendingWeight = ascendingSegments.reduce((sum, s) =>
      sum + s.significance * (s.end - s.start + 1), 0);
    const descendingWeight = descendingSegments.reduce((sum, s) =>
      sum + s.significance * (s.end - s.start + 1), 0);
    const staticWeight = staticSegments.reduce((sum, s) =>
      sum + s.significance * (s.end - s.start + 1), 0);
  ...
  private calculateTessitura(notes: number[]): {
    ...
    const median = sortedNotes.length % 2 === 0
    ...
    const notesInTessitura = notes.filter(note =>
      note >= tessituraStart && note <= tessituraEnd
    ).length;
   ...
   */
  private detectMusicalPhrases(notes: number[], intervals: number[]): Array<{
    start: number;
    end: number;
    ...
    for (let i = 1; i < notes.length - 1; i++) {
      ...
      const isPhraseEnd =
        ...
        (Math.abs(currentInterval) >= 4 && Math.abs(nextInterval) <= 2 &&
  ...
  private calculateMelodicComplexity(
    ...
    const intervalComplexity = intervals.length > 0
      ? uniqueIntervals / intervals.length
      : 0;
    ...
    const contourComplexity = intervals.length > 1
      ? directionChanges / (intervals.length - 1)
      : 0;
   ...
   */
  private analyzeMotifs(notes: number[], intervals: number[]): {
      ...
      for (let start = 0; start <= notes.length - length * 2; start++) {
        ...
        for (let searchStart = start + 1; searchStart <= notes.length - length; searchStart++) {
          ...
          if (JSON.stringify(pattern) === JSON.stringify(candidate) ||
              JSON.stringify(patternIntervals) === JSON.stringify(candidateIntervals)) {
            frequency++;
          }
        }
        ...
        if (frequency > 1) {
          ...
          const existingMotif = motifs.find(m =>
            JSON.stringify(m.pattern) === JSON.stringify(pattern)
          );
    ...
    const repeatedNotes = motifs.reduce((sum, motif) =>
      sum + motif.pattern.length * (motif.frequency - 1), 0
    );
    const repetition = totalNotes > 0 ? repeatedNotes / totalNotes : 0;

    // Development score based on variation of motifs
    const development = motifs.length > 0
