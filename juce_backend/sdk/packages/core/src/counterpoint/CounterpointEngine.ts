...
import { Scale, Chord, Note, Interval } from '../theory/types';
...
export interface VoiceLeadingConstraints {
  ...
  forbiddenIntervals: Interval[]; // Intervals that should not occur
  requiredIntervals: Interval[];  // Intervals that must occur
...
export class CounterpointEngine {
  ...
  async generateCounterpoint(
    ...
    return {
      notes: refinedNotes,
      name: `Counterpoint ${rules.species}th Species`,
  ...
  async generatePolyphonicTexture(
    ...
    let previousVoice: VoicePart | undefined;

    for (let voice = 0; voice < voiceCount; voice++) {
      const adaptedRules = this.adaptRulesForVoice(rules, voice, voiceCount);
      const referenceVoice = voice === 0 ? cantusFirmus : (previousVoice || cantusFirmus);

      const counterpoint = await this.generateCounterpoint(referenceVoice, adaptedRules);
      counterpoints.push(counterpoint);
      previousVoice = counterpoint;
    }
  ...
  async analyzeCounterpoint(
    ...
    const intervals = this.calculateHarmonicIntervals(cantusFirmus.notes, counterpoint.notes);

    return {
      validity: await this.validateCounterpoint(cantusFirmus, counterpoint, rules),
      ...
      harmonicAnalysis: {
        ...
        suspensions: this.countSuspensions(cantusFirmus.notes, counterpoint.notes),
        resolutions: this.countResolutions(cantusFirmus.notes, counterpoint.notes),
      },
      rhythmicAnalysis: {
  ...
  applyVoiceLeading(
    ...
    for (let i = 0; i < Math.min(sourceNotes.length, targetNotes.length); i++) {
      ...
      if (this.containsInterval(constraints.forbiddenIntervals, melodicInterval)) {
        return false;
      }
      ...
      if (!constraints.voiceCrossing && i > 0) {
        ...
        if (this.checkVoiceCrossing(sourceNote, targetNote, prevSource, prevTarget)) {
          return false;
        }
      }
    }
  ...
  private validateInput(cantusFirmus: VoicePart, rules: CounterpointRules): void {
    ...
    for (const note of cantusFirmus.notes) {
      if (note.midi < rules.cantusFirmusRange[0] || note.midi > rules.cantusFirmusRange[1]) {
        throw new Error(`Cantus firmus note ${note.midi} outside allowed range`);
      }
    }
  }
  ...
  private async generateNoteForCantus(
    ...
    if (filteredCandidates.length === 0) {
      throw new Error(`No valid counterpoint note found for cantus note at index ${index}`);
    }
  ...
  private generateCandidateNotes(cantusNote: Note, rules: CounterpointRules): Note[] {
  ...
  private isIntervalAllowed(interval: number, rules: CounterpointRules): boolean {
    const normalizedInterval = interval % 12 || 12;

    // Check forbidden intervals
    if (this.containsInterval(rules.constraints.forbiddenIntervals, normalizedInterval)) {
      return false;
    }

    // Check if it's a consonance (generally preferred)
    const isConsonance = this.PERFECT_CONSONANCES.includes(normalizedInterval) ||
  ...
  private filterCandidates(
    ...
    return candidates.filter(candidate => {
      ...
      if (index > 0) {
        ...
        if (!this.applyVoiceLeading([previousCounterpoint], [candidate], rules.constraints)) {
          return false;
        }

        // Check parallel motion limits
        if (this.isParallelMotion(previousCantus, cantusNote, previousCounterpoint, candidate)) {
          // TODO: Count consecutive parallel motions and check limit
        }
      }

      // Check melodic contour (prefer contrary motion)
      if (index > 0 && this.hasSameDirection(cantusNotes[index - 1], cantusNote, counterpointNotes[index - 1], candidate)) {
  ...
  private selectBestCandidate(candidates: Note[], cantusNote: Note, rules: CounterpointRules): Note {
  ...
  private calculateCandidateScore(candidate: Note, cantusNote: Note, rules: CounterpointSpecies): number {
    let score = 0;
    ...
    const rangeCenter = (rules.counterpointRange[0] + rules.counterpointRange[1]) / 2;
  ...
  private calculateHarmonicIntervals(cantusNotes: Note[], counterpointNotes: Note[]): Interval[] {
    const intervals: Interval[] = [];
    const minLength = Math.min(cantusNotes.length, counterpointNotes.length);

    for (let i = 0; i < minLength; i++) {
      const interval = Math.abs(counterpointNotes[i].midi - cantusNotes[i].midi) % 12 || 12;
      intervals.push(interval as Interval);
    }

    return intervals;
  }

  private countConsonances(intervals: Interval[]): number {
  ...
  private countDissonances(intervals: Interval[]): number {
    return intervals.filter(interval =>
      this.DISSONANCES.includes(interval)
    ).length;
  }

  private countSuspensions(cantusNotes: Note[], counterpointNotes: Note[]): number {
    // TODO: Implement suspension detection
    return 0;
  }

  private countResolutions(cantusNotes: Note[], counterpointNotes: Note[]): number {
  ...
  private checkSpeciesConformance(counterpoint: VoicePart, rules: CounterpointRules): boolean {
    // TODO: Implement species-specific validation
    return true;
  }

  private calculateVoiceLeadingScore(counterpoint: VoicePart, rules: CounterpointRules): number {
  ...
  private adaptRulesForVoice(rules: CounterpointRules, voiceIndex: number, totalVoices: number): CounterpointRules {
    // TODO: Adapt constraints based on voice position
    return { ...rules };
  }

  private intersectPatterns(pattern1: number[], pattern2: number[]): number[] {
    // TODO: Implement Schillinger pattern intersection
    return pattern1;
  }
  ...
  private containsInterval(intervals: Interval[], target: number): boolean {
    return intervals.includes(target as Interval);
  }
  ...
  private isParallelMotion(
    ...
    const counterpointDirection = counterpoint2.midi > counterpoint1.midi ? 1 : -1;
  ...
  private hasSameDirection(
    ...
    return this.isParallelMotion(cantus1, cantus2, counterpoint1, counterpoint2);
  }
  ...
  private checkVoiceCrossing(
    ...
    return (source1.midi < source2.midi && target1.midi > target2.midi) ||
