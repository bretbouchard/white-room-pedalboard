...
import { CounterpointEngine, CounterpointSpecies, VoicePart, CounterpointRules } from '../CounterpointEngine';
...
describe('CounterpointEngine', () => {
  ...
  describe('Input Validation', () => {
    it('should reject empty cantus firmus', async () => {
      ...
      await expect(engine.generateCounterpoint(cantusFirmus, rules))
    ...
    it('should generate simple first species counterpoint', async () => {
      ...
      const rules: CounterpointRules = {
        ...
        constraints: {
          maxMelodicInterval: 8,
          maxHarmonicInterval: 12,
          forbiddenIntervals: [4], // Tritone forbidden
      ...
      const intervals = calculateIntervals(cantusFirmus.notes, counterpoint.notes);
  ...
  describe('Polyphonic Texture Generation', () => {
    it('should generate two-part polyphony', async () => {
      ...
      const counterpoints = await engine.generatePolyphonicTexture(cantusFirmus, 2, rules);
    ...
    it('should reject invalid voice count', async () => {
      ...
      await expect(engine.generatePolyphonicTexture(cantusFirmus, 0, rules))
  ...
  describe('Rhythmic Pattern Generation', () => {
    it('should generate rhythmic resultants', () => {
      ...
      const patterns = engine.generateRhythmicPatterns(basePattern, resultantPattern, 3);
    ...
    it('should handle pattern rotation correctly', () => {
      ...
      const resultantPattern = [1, 1, 0, 1];

      const patterns = engine.generateRhythmicPatterns(basePattern, resultantPattern, 4);
  ...
  describe('Voice Leading', () => {
    it('should apply correct voice leading constraints', () => {
      ...
      const result = engine.applyVoiceLeading(sourceNotes, targetNotes, constraints);
  ...
  describe('Counterpoint Analysis', () => {
    it('should analyze basic counterpoint correctly', async () => {
      ...
      const analysis = await engine.analyzeCounterpoint(cantusFirmus, counterpoint, rules);
  ...
  describe('Property-Based Testing', () => {
    it('should generate valid counterpoint for any valid cantus', async () => {
      await fc.assert(
        fc.property(
          fc.array(SchillingerArbitraries.note, { minLength: 4, maxLength: 16 }),
          ...
          async (notes, species) => {
            ...
            const rules: CounterpointRules = {
              ...
              counterpointRange: [Math.min(...notes) + 12, Math.max(...notes) + 24],
            };
            ...
            try {
              ...
              counterpoint.notes.forEach(note => {
                expect(note.midi).toBeGreaterThanOrEqual(rules.counterpointRange[0]);
                expect(note.midi).toBeLessThanOrEqual(rules.counterpointRange[1]);
              });
    ...
    it('should satisfy associative property for pattern combination', () => {
      PropertyTestHelpers.associative(
        (a, b, c) => (a + b + c) % 8,
        (a, b) => a === b
      );
    });
