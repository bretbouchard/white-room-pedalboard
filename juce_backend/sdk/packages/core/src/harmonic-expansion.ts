...
export type HarmonicExpansionType =
  | 'parallel' | 'contrary' | 'oblique' | 'mixed' | 'rotational' | 'retrograde' |
    'invertible' | 'pandiatonic' | 'chromatic' | 'polychordal' | 'tension_based' | 'functional';
...
export interface ExpansionParameters {
  ...
  voiceLeadingConstraints?: VoiceLeadingConstraints;
...
export class HarmonicExpansionEngine {
  ...
  private static createRotationalExpansion(
    ...
    const pitchIntervals = harmony.map((element, index) =>
      index < harmony.length - 1 ? harmony[index + 1].pitch - element.pitch : harmony[0].pitch - element.pitch
    );
  ...
  private static createChromaticExpansion(
    ...
    return harmony.map(element => ({
      ...
      tension: 0.8 + Math.random() * 0.2, // High tension for chromatic
      function: 'chromatic',
  ...
  static generatePolychord(
    chords: HarmonicElement[][],
    intervals: number[] = [0, 7, 12] // Default: stacked thirds
  ): Polychord {
    ...
    chords.forEach((chord, chordIndex) => {
      ...
      currentInterval += intervals[chordIndex] || intervals[0];
    });
  ...
  static optimizeVoiceLeading(
    ...
    while (improved && iterations < maxIterations) {
      ...
      for (let i = 1; i < optimized.length; i++) {
        ...
        let bestPitch = currentElement.pitch;
        let bestScore = this.calculateVoiceLeadingScore(prevElement, currentElement, constraints);

        // Test alternative pitches within constraint limits
        const maxLeap = constraints.maxLeap || 12;
        const testPitches = [
          currentElement.pitch - 2, // Down step
          currentElement.pitch - 1, // Down half step
          currentElement.pitch + 1, // Up half step
          currentElement.pitch + 2  // Up step
        ].filter(pitch =>
        ...
        if (bestPitch !== currentElement.pitch) {
  ...
  private static calculateIntervalTension(pitch1: number, pitch2: number): number {
  ...
  private static calculateVoiceLeadingTension(harmony: HarmonicElement[]): number {
  ...
  private static calculateFunctionalTensionAverage(harmony: HarmonicElement[]): number {
    ...
    return harmony.reduce((sum, element) =>
      sum + (functionalTensions[element.function as keyof typeof functionalTensions] || 0.5),
    0) / harmony.length;
  }
  ...
  private static calculateStructuralStability(harmony: HarmonicElement[]): number {
  ...
  private static calculatePolychordClarity(combined: HarmonicElement[], numChords: number): number {
  ...
  private static calculateTonalTension(pitch: number, root: number, scale: number[]): number {
  ...
  private static calculateTensionProfile(harmony: HarmonicElement[]): TensionModel[] {
  ...
  private static analyzeFunctions(harmony: HarmonicElement[]): {
    ...
    const functions = [...new Set(progression)];
...
export class HarmonicExpansionAPI {
  ...
  static async generateHarmonicExpansion(
    ...
    const alternativeTypes: HarmonicExpansionType[] = ['parallel', 'contrary', 'mixed']
      .filter(type => type !== expansionType);
    const alternativeExpansions = alternativeTypes.slice(0, 2).map(type =>
   ...
   * Analyze complete harmonic structure
   */
  static async analyzeCompleteHarmony(
    harmony: HarmonicElement[]
  ): Promise<{
    ...
    const qualityAssessment = {
      overall: analysis.structural.complexity * 0.3 +
  ...
  static async optimizeHarmony(
    ...
    const improvement = {
      voiceLeadingScore: optimizedTension.voiceLeading - originalTension.voiceLeading,
      tensionImprovement: originalTension.total - optimizedTension.total,
  ...
  private static generateRecommendations(expansion: HarmonicExpansion): string[] {
