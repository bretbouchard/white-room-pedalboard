...
export interface FormalSection {
  id: string;
  name: string;
  type: 'exposition' | 'development' | 'recapitulation' | 'transition' | 'bridge' | 'coda' | 'episode' | 'theme' | 'variation' | 'refrain' | 'prelude' | 'postlude';
  duration: Rational;
  ...
  structure: {
    ...
    cadenceType: 'perfect' | 'imperfect' | 'plagal' | 'deceptive' | 'half' | 'none';
...
export interface MusicalForm {
  id: string;
  name: string;
  type: 'binary' | 'ternary' | 'sonata' | 'rondo' | 'arch' | 'theme_variations' | 'through_composed' | 'strophic' | 'custom';
...
export interface TonalPlan {
  ...
  modulationStrategy: 'direct' | 'prepared' | 'common_chord' | 'enharmonic' | 'chromatic';
...
export interface KeyRelationship {
  from: string;
  to: string;
  relationship: 'parallel' | 'relative' | 'dominant' | 'subdominant' | 'mediant' | 'chromatic' | 'enharmonic';
...
export interface ThemeTransformation {
  type: 'inversion' | 'retrograde' | 'augmentation' | 'diminution' | 'fragmentation' | 'development' | 'sequencing';
...
export interface FormalFunction {
  id: string;
  sectionId: string;
  function: 'tonic' | 'dominant' | 'subdominant' | 'pre_dominant' | 'transition' | 'development' | 'recapitulation' | 'coda';
...
export interface StructuralConnection {
  from: string;
  to: string;
  type: 'direct' | 'bridge' | 'transition' | 'modulation' | 'elision' | 'overlap';
...
export class FormEngine {
  ...
  static transformForm(
    form: MusicalForm,
    transformation: 'expand' | 'contract' | 'develop' | 'simplify' | 'contemporize',
    parameters: {
    ...
    switch (transformation) {
      case 'expand':
        ...
        transformedDuration = form.duration.mul(new Rational(Math.floor(expansionFactor * 1000), 1000));
        break;

      case 'contract':
        const contractionFactor = parameters.factor || 0.7;
        transformedSections = this.contractForm(form.sections, contractionFactor);
        transformedDuration = form.duration.mul(new Rational(Math.floor(contractionFactor * 1000), 1000));
        break;
  ...
  private static generateSections(
    ...
    template.structure.forEach((sectionTemplate, index) => {
      const sectionDuration = duration.mul(new Rational(Math.floor(sectionTemplate.relativeDuration * 1000), 1000));
  ...
  private static generateSectionCharacter(
    ...
    Object.keys(baseCharacter).forEach(key => {
      baseCharacter[key as keyof typeof baseCharacter] = Math.max(0, Math.min(1, baseCharacter[key as keyof typeof baseCharacter]));
    });
  ...
  private static generateSubsectionCharacter(index: number, total: number): FormalSection['character'] {
  ...
  private static setupSectionRelationships(sections: FormalSection[]): void {
    sections.forEach((section, index) => {
      ...
      if (index > 0) {
        section.follows.push(sections[index - 1].id);
        sections[index - 1].precedes.push(section.id);
      }

      if (index < sections.length - 1) {
        section.precedes.push(sections[index + 1].id);
        sections[index + 1].follows.push(section.id);
      }
      ...
      sections.forEach((otherSection, otherIndex) => {
        if (index !== otherIndex) {
          if (section.type === otherSection.type) {
            section.parallels.push(otherSection.id);
          } else if (this.areContrastingTypes(section.type, otherSection.type)) {
            section.contrasts.push(otherSection.id);
          }
        }
      });
    });
  ...
  private static areContrastingTypes(type1: FormalSection['type'], type2: FormalSection['type']): boolean {
    ...
    return contrastingPairs.some(pair =>
      (pair.includes(type1) && pair.includes(type2)) ||
      (pair.some(p => type1.includes(p)) && pair.some(p => type2.includes(p)))
    );
  }
  ...
  private static generateKeyRelationships(
    ...
    const keys = [primary, secondary, relative, dominant].filter(Boolean) as string[];
  ...
  private static generateThemeTransformations(constraints: FormGenerationConstraints): ThemeTransformation[] {
  ...
  private static analyzeStructuralConnections(sections: FormalSection[]): StructuralConnection[] {
  ...
  private static expandForm(sections: FormalSection[], factor: number): FormalSection[] {
    return sections.map(section => ({
      ...section,
      duration: section.duration.mul(new Rational(Math.floor(factor * 1000), 1000)),
      structure: {
        ...section.structure,
        subsections: section.structure.subsections.map(subsection => ({
          ...subsection,
          duration: subsection.duration.mul(new Rational(Math.floor(factor * 1000), 1000))
        }))
      }
    }));
  }

  private static contractForm(sections: FormalSection[], factor: number): FormalSection[] {
    return sections.map(section => ({
      ...section,
      duration: section.duration.mul(new Rational(Math.floor(factor * 1000), 1000)),
      structure: {
  ...
  private static developForm(sections: FormalSection[], intensity: number): FormalSection[] {
    return sections.map(section => ({
      ...section,
      character: {
        ...section.character,
  ...
  private static simplifyForm(sections: FormalSection[], intensity: number): FormalSection[] {
    return sections.map(section => ({
      ...section,
      character: {
        ...section.character,
  ...
  private static contemporizeForm(sections: FormalSection[], elements: string[]): FormalSection[] {
    return sections.map(section => ({
      ...section,
...
export class FormAPI {
  ...
  static generateFormVariations(
    form: MusicalForm,
    count: number = 3,
    variationType: 'structural' | 'thematic' | 'developmental' | 'combined' = 'combined'
  ): MusicalForm[] {
  ...
  // Private helper methods
  private static groupByHarmonicFunction(harmonicAnalysis: Array<{ time: number; key: string; function: string }>) {
  ...
  private static inferSectionType(function_: string, index: number): FormalSection['type'] {
    switch (function_) {
      case 'tonic': return 'theme';
      case 'dominant': return 'transition';
  ...
  private static calculateStability(function_: string): number {
    switch (function_) {
      case 'tonic': return 0.9;
      case 'dominant': return 0.4;
      case 'subdominant': return 0.7;
  ...
  private static calculateStructuralSimilarity(form1: MusicalForm, form2: MusicalForm): number {
  ...
  private static calculateThematicSimilarity(form1: MusicalForm, form2: MusicalForm): number {
  ...
  private static findFormDifferences(form1: MusicalForm, form2: MusicalForm): string[] {
    const differences = [];
  ...
  private static findFormRelationships(form1: MusicalForm, form2: MusicalForm): string[] {
    const relationships = [];
