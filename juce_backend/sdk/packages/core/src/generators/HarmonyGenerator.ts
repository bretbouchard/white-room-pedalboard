...
export interface ResolutionParams {
  key: string;
  scale: string;
  context?: 'dominant_preparation' | 'tonic_preparation' | 'subdominant_preparation';
...
export interface HarmonyVariationParams {
  style: 'modal_interchange' | 'secondary_dominants' | 'tritone_substitution' | 'color_chords';
...
> {
   ...
   */
  generateProgression(params: ProgressionParams): GeneratorResult<ChordProgression> {
    ...
    try {
      ...
      const progression = sdk.harmony.generateProgression(
        params.key,
        params.scale,
        params.length,
        {
  ...
  analyzeProgression(
  ...
  ): GeneratorResult<HarmonicAnalysis> {
    ...
    try {
      ...
      // Delegate to existing functional API
      const analysis = sdk.harmony.analyzeProgression(progression.chords);
  ...
  generateVariations(
  ...
  ): GeneratorResult<ChordProgression> {
    const methodParams = {
    ...
    try {
      ...
      const variation = sdk.harmony.generateVariations(progression, params);
  ...
  resolveChord(
    ...
    try {
      ...
      // Delegate to existing functional API
      const resolution = sdk.harmony.resolveChord(chord, {
        key: mergedParams.key || params.key,
        scale: mergedParams.scale || params.scale,
  ...
  inferHarmonicStructure(
    ...
    try {
      ...
      // Delegate to existing functional API (if available)
      const structure = sdk.harmony.inferHarmonicStructure(chords);
