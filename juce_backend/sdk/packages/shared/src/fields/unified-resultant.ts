...
import { GeneratorPair, calculateGCD } from '../math/generators';

/**
 * Resultant combination strategies
 */
export type CombinationStrategy =
  | 'additive'        // Simple overlay
  | 'multiplicative'  // Cross-synthesis
  | 'convolution'     // Pattern convolution
  | 'emergent'        // Emergent behavior
  | 'hierarchical'    // Role-based hierarchy
...
export class UnifiedResultant implements IUnifiedResultant {
  ...
  getEmergenceAnalysis(): EmergenceAnalysis {
    return this._emergenceAnalysis || {
      hasEmergence: false,
      emergenceScore: 0,
      emergentPatterns: [],
      novelty: 0,
  ...
  private generateAdditive(): void {
    ...
    for (const [layerId, layerMaterial] of layers) {
      for (const event of layerMaterial) {
        ...
        const conflict = combined.find(e =>
  ...
  private generateMultiplicative(): void {
    ...
    for (const baseEvent of baseLayer) {
      ...
      const relatedEvents = otherLayers.map(layer =>
      ...
      if (relatedEvents.length > 0) {
        ...
        const avgAmplitude = [baseEvent, ...relatedEvents]
          .reduce((sum, e, _, arr) => sum + e.amplitude / arr.length, 0);

        const combinedDuration = Math.max(baseEvent.duration, ...relatedEvents.map(e => e.duration));
        ...
        combined.push({
          ...
          articulation: {
            attack: Math.max(baseEvent.articulation?.attack || 0.5, ...relatedEvents.map(e => e.articulation?.attack || 0.5)),
            release: Math.max(baseEvent.articulation?.release || 0.5, ...relatedEvents.map(e => e.articulation?.release || 0.5)),
            sustain: (baseEvent.articulation?.sustain || 0.5 + relatedEvents.reduce((sum, e) => sum + (e.articulation?.sustain || 0.5), 0)) / (relatedEvents.length + 1)
          },
          metadata: {
            generatorId: 'multiplicative',
            originalLayers: [baseEvent.metadata?.generatorId, ...relatedEvents.map(e => e.metadata?.generatorId)].filter(Boolean)
          }
        });
      } else {
  ...
  private generateConvolution(): void {
    ...
    for (let i = 0; i < pattern1.length; i++) {
      for (let j = 0; j < pattern2.length; j++) {
        ...
        const convolvedPitch = event1.pitch ? Math.round(event1.pitch + pitchModulation * 2) : undefined;
        ...
        convolved.push({
          ...
          metadata: {
            ...
            originalEvents: [event1.id, event2.id]
          }
        });
      }
    }
  ...
  private generateEmergent(): void {
    ...
    for (let i = 0; i < layers.length; i++) {
      ...
      for (let j = i + 1; j < layers.length; j++) {
        ...
        const crossEmergents = this.findCrossLayerEmergence(layer1, layer2, analysis1, layerAnalyses[j]);
    ...
    for (const [layerId, layerMaterial] of layers) {
      for (const event of layerMaterial) {
        ...
        if (!conflict) {
          combined.push({
            ...event,
            amplitude: event.amplitude * this.getHierarchicalWeight(layerId, layerMaterial),
            metadata: {
              ...event.metadata,
              generatorId: layerId,
  ...
  private generateProbabilistic(): void {
    ...
    const totalProb = layerProbabilities.reduce((sum, lp) => sum + lp.probability, 0);
    layerProbabilities.forEach(lp => lp.probability /= totalProb);

    // Stochastically select events
    for (const [layerId, layerMaterial] of layers) {
      const layerProb = layerProbabilities.find(lp => lp.id === layerId)?.probability || 0;
  ...
  private analyzeCoherence(): void {
    ...
    this.coherence = (rhythmicCoherence + harmonicCoherence + melodicCoherence + structuralCoherence) / 4;
  }
  ...
  private analyzeEmergence(): void {
    ...
    const emergenceScore = emergentPatterns.reduce((sum, p) => sum + p.confidence, 0) / Math.max(emergentPatterns.length, 1);
  ...
  private analyzeLayerContributions(): void {
    ...
    for (const [layerId, layerMaterial] of this._layers.entries()) {
      const contribution = this.analyzeSingleLayerContribution(layerId, layerMaterial);
  ...
  private analyzeLayer(layer: MusicalMaterial): any {
    ...
    return {
      density: layer.length / Math.max(...layer.map(e => e.time + e.duration), 1),
      complexity: this.calculateComplexity(layer),
      averagePitch: layer.reduce((sum, e) => sum + (e.pitch || 0), 0) / layer.length
    };
  }

  private applyEmergentModifications(event: MusicalEvent, layer: MusicalMaterial, layerIndex: number): MusicalEvent {
    // Apply context-aware modifications
    const context = this.getEventContext(event, layer);
    const modifications = this.calculateEmergentModifications(context, layerIndex);

    return {
      ...event,
      amplitude: Math.max(0, Math.min(1, event.amplitude * modifications.amplitude)),
      articulation: {
        attack: Math.max(0, Math.min(1, (event.articulation?.attack || 0.5) + modifications.attack)),
        release: Math.max(0, Math.min(1, (event.articulation?.release || 0.5) + modifications.release)),
        sustain: Math.max(0, Math.min(1, (event.articulation?.sustain || 0.5) + modifications.sustain))
      }
    };
  }

  private findCrossLayerEmergence(layer1: MusicalMaterial, layer2: MusicalMaterial, analysis1: any, analysis2: any): MusicalMaterial {
  ...
  private inferLayerRole(layer: MusicalMaterial): MusicalRole {
    ...
    const pitches = layer.filter(e => e.pitch !== undefined).map(e => e.pitch!);
    const avgPitch = pitches.length > 0 ? pitches.reduce((sum, p) => sum + p, 0) / pitches.length : 60;
  ...
  private calculateHierarchicalWeight(layerId: string, layer: MusicalMaterial): number {
  ...
  private calculatePitchConflict(event1: MusicalEvent, event2: MusicalEvent): number {
  ...
  private analyzeRhythmicCoherence(): number {
    ...
    const intervals = this.material.slice(1).map((e, i) => e.time - this.material[i].time);
    const variance = intervals.reduce((sum, interval, _, arr) => {
  ...
  private analyzeHarmonicCoherence(): number {
    ...
    const pitches = this.material.filter(e => e.pitch !== undefined).map(e => e.pitch!);
  ...
  private findEmergentPatterns(): Array<{ type: string; description: string; confidence: number; location: number }> {
  ...
  private analyzeSingleLayerContribution(layerId: string, layer: MusicalMaterial): LayerContribution {
    return {
      layerId,
  ...
  private calculateComplexity(layer: MusicalMaterial): number {
  ...
  private calculateEmergentModifications(context: any, layerIndex: number): any {
    return {
  ...
  private patternsAreRelated(pattern1: MusicalEvent[], pattern2: MusicalEvent[]): boolean {
    // Simplified pattern relationship check
    if (pattern1.length !== pattern2.length) return false;

    const intervalPattern1 = pattern1.slice(1).map((e, i) => e.time - pattern1[i].time);
    const intervalPattern2 = pattern2.slice(1).map((e, i) => e.time - pattern2[i].time);

    return JSON.stringify(intervalPattern1) === JSON.stringify(intervalPattern2);
  }

  private createEmergentEvent(pattern1: MusicalEvent[], pattern2: MusicalEvent[]): MusicalEvent {
    return {
      id: this.generateEventId(),
  ...
  private patternsMatch(pattern1: MusicalEvent[], pattern2: MusicalEvent[]): boolean {
    if (pattern1.length !== pattern2.length) return false;

    return pattern1.every((event1, i) => {
      const event2 = pattern2[i];
      return Math.abs(event1.time - event2.time) < 0.1 &&
             Math.abs(event1.duration - event2.duration) < 0.1 &&
             (event1.pitch === event2.pitch || event1.pitch === undefined || event2.pitch === undefined);
    });
  }
}