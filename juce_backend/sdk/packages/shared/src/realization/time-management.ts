...
import { MusicalTime, TimeRange } from '../types/realization';
...
export class TimeConverter {
   ...
   */
  static getMeasureBeat(time: MusicalTime, tempoMeter: TempoMeter): {
    measure: number;
    beat: number;
    beatPosition: number; // Position within beat 0.0-1.0
  } {
    const totalBeats = time.beats || this.secondsToBeats(time.seconds, tempoMeter.tempo);
    const beatsPerMeasure = tempoMeter.meter[0];
...
export class MusicalTimeRange implements TimeRange {
  public start: MusicalTime;
  public end: MusicalTime;
  public duration: number;

  constructor(start: MusicalTime, end: MusicalTime) {
    ...
    if (this.start.beats === undefined && this.start.tempo) {
      this.start.beats = TimeConverter.secondsToBeats(this.start.seconds, this.start.tempo);
    }
    if (this.end.beats === undefined && this.end.tempo) {
      this.end.beats = TimeConverter.secondsToBeats(this.end.seconds, this.end.tempo);
    }
  }
  ...
  contains(time: MusicalTime): boolean {
    return time.seconds >= this.start.seconds && time.seconds <= this.end.seconds;
  }
    ...
    return new MusicalTimeRange(
      {
        ...
        beats: this.start.beats ? this.start.beats + TimeConverter.secondsToBeats(delta, 120) : undefined,
        precision: 'seconds'
      },
      {
        seconds: this.end.seconds + delta,
        beats: this.end.beats ? this.end.beats + TimeConverter.secondsToBeats(delta, 120) : undefined,
        precision: 'seconds'
      }
    );
  }
  ...
  split(chunkDuration: number): MusicalTimeRange[] {
    ...
    while (currentTime < this.end.seconds) {
      ...
      chunks.push(new MusicalTimeRange(
        {
          seconds: currentTime,
          precision: 'seconds'
        },
...
export class TimeScheduler {
   ...
   */
  scheduleRecurring(startTime: number, interval: number, callback: () => void): string {
...
export class TimeUtils {
   ...
   * Clamp time to range
   */
  static clampTime(time: MusicalTime, min: number, max: number): MusicalTime {
    return {
      ...time,
   ...
   */
  static lerpTime(time1: MusicalTime, time2: MusicalTime, t: number): MusicalTime {
    return {
      seconds: time1.seconds + (time2.seconds - time1.seconds) * t,
      beats: time1.beats && time2.beats ?
        time1.beats + (time2.beats - time1.beats) * t : undefined,
   ...
   */
  static timeEquals(time1: MusicalTime, time2: MusicalTime, tolerance: number = 0.001): boolean {
   ...
   */
  static formatTime(time: MusicalTime, showBeats: boolean = false): string {
   ...
   */
  static fromSeconds(seconds: number, tempo?: number): MusicalTime {
    return {
      seconds,
   ...
   */
  static fromBeats(beats: number, tempo: number): MusicalTime {
    return {
