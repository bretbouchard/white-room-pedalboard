...
import { MusicalTime } from '../types/realization';
...
export class RealizedLayer implements IRealizedLayer {
  ...
  process(context: LayerContext): void {
    ...
    if (this.processingOptions.enableDynamicRange) {
      processedMaterial = this.applyDynamicRange(processedMaterial, context.intensity);
    }
  ...
  addEvents(events: MusicalEvent[]): void {
    ...
    // Record processing
    this.recordProcessing('addEvents', { count: events.length, valid: filteredEvents.length });
  }
  ...
  removeEvents(eventIds: string[]): void {
    ...
    this.material = this.material.filter(event => !eventIds.includes(event.id));
  ...
  getEventsInRange(startTime: number, endTime: number): MusicalEvent[] {
    return this.material.filter(event =>
  ...
  getStatistics(): {
    ...
    const registerUtilization = pitchRange ?
      Math.min(1, pitchRange.range / (this.register.width || 24)) : 0;

    return {
      eventCount: this.material.length,
      duration,
      density: this.material.length / Math.max(duration, 1),
      averageAmplitude: this.material.reduce((sum, e) => sum + e.amplitude, 0) / this.material.length,
      averageDuration: this.material.reduce((sum, e) => sum + e.duration, 0) / this.material.length,
      pitchRange,
      registerUtilization
    }
  }
  ...
  validateCoherence(): {
    ...
    if (outOfRegisterEvents.length > 0) {
      issues.push(`${outOfRegisterEvents.length} events outside register range`);
      suggestions.push('Consider transposing events or adjusting register');
    }

    // Check for very short events
    const shortEvents = this.material.filter(event => event.duration < this.processingOptions.minEventDuration);
    if (shortEvents.length > 0) {
      issues.push(`${shortEvents.length} events too short for practical performance`);
      suggestions.push('Increase minimum event duration or merge short events');
    }

    // Check for very long events
    const longEvents = this.material.filter(event => event.duration > this.processingOptions.maxEventDuration);
    if (longEvents.length > 0) {
      issues.push(`${longEvents.length} events may be too long for musical context`);
  ...
  private applyQuantization(material: MusicalMaterial): MusicalMaterial {
    ...
    return material.map(event => ({
      ...
      duration: Math.max(this.processingOptions.minEventDuration,
        Math.round(event.duration / grid) * grid)
    }));
  }

  private applyDynamicRange(material: MusicalMaterial, contextIntensity: number): void {
    ...
    for (const event of material) {
      ...
      const compressedAmplitude = targetAmplitude * (1 - compress) + 0.5 * compress;
  ...
  private applySmoothing(material: MusicalMaterial): MusicalMaterial {
    ...
    for (let i = 1; i < material.length; i++) {
      ...
      const smoothedAmplitude = prev.amplitude * this.processingOptions.smoothingFactor +
      ...
      smoothed.push({
        ...
        articulation: current.articulation ? {
          ...
          attack: prev.articulation?.attack ?
            prev.articulation.attack * this.processingOptions.smoothingFactor +
            current.articulation.attack * (1 - this.processingOptions.smoothingFactor) :
  ...
  private blendMaterials(existing: MusicalEvent[], newEvents: MusicalEvent[]): MusicalEvent[] {
    const blended = [...existing];

    for (const newEvent of newEvents) {
      // Find overlapping events
      const overlappingIndex = blended.findIndex(existing =>
  ...
  private isValidEvent(event: MusicalEvent): boolean {
    ...
    // Duration validation
    if (event.duration < this.processingOptions.minEventDuration ||
        event.duration > this.processingOptions.maxEventDuration) {
      return false;
    }

    // Amplitude validation
    if (event.amplitude < 0 || event.amplitude > 1 || !Number.isFinite(event.amplitude)) {
      return false;
    }
  ...
  private calculateComplexity(stats: any): number {
    ...
    const pitchRangeFactor = stats.pitchRange ? Math.min(1, stats.pitchRange.range / 24) : 0;
  ...
  private recordProcessing(operation: string, parameters?: Record<string, any>): void {
