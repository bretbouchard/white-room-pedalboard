  ...
  private async makeAuthRequest(
    ...
    for (let attempt = 1; attempt <= this.retryConfig.maxRetries; attempt++) {
      try {
        ...
        const fetchFn: any = (globalThis as any).fetch || (typeof fetch !== 'undefined' ? fetch : undefined);
        ...
        if (response == null && (fetchFn as any)?.mock && Array.isArray((fetchFn as any).mock.results)) {
