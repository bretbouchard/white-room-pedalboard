...
import { CounterpointEngine, CounterpointSpecies, VoicePart, CounterpointRules } from '../../../packages/core/src/counterpoint/CounterpointEngine';
...
describe('CounterpointEngine Performance Benchmarks', () => {
  ...
  describe('Real-Time Requirements', () => {
    it('should generate 4-note first species counterpoint in under 1ms', async () => {
      ...
      const { result, metrics } = await PerformanceTestHelpers.assertPerformance(
  ...
  describe('Mathematical Operations Performance', () => {
    ...
    it('should scale linearly with cantus firmus length', async () => {
      ...
      for (const length of lengths) {
        ...
        const { metrics } = await PerformanceTestHelpers.benchmarkFunction(
          () => engine.generateCounterpoint(cantusFirmus, rules),
          'mathematical-operations',
          10
        );

        executionTimes.push(metrics.meanTime);
      }
  ...
  describe('Memory Efficiency', () => {
    it('should maintain stable memory usage for repeated operations', async () => {
      ...
      expect(report.summary.standardDeviation).toBeLessThan(report.summary.meanTime * 0.5);
    });
    ...
    it('should not leak memory during polyphonic generation', async () => {
      ...
      for (const voiceCount of voiceCounts) {
        ...
        memoryUsages.push(metrics.memoryIncrease);
      }
  ...
  describe('Pattern Generation Performance', () => {
    it('should generate rhythmic patterns efficiently', () => {
      const basePattern = [1, 0, 1, 0, 1, 0, 1, 0];
      const resultantPattern = [1, 1, 0, 1, 1, 0, 1, 1];

      const { metrics } = PerformanceTestHelpers.assertPerformance(
    ...
    it('should handle complex pattern intersections', () => {
      const largePattern = Array.from({ length: 32 }, (_, i) => i % 3 === 0 ? 1 : 0);
      const complexPattern = Array.from({ length: 32 }, (_, i) => i % 5 === 0 || i % 7 === 0 ? 1 : 0);
  ...
  describe('Voice Leading Performance', () => {
    it('should validate voice leading constraints quickly', () => {
    ...
    it('should handle complex constraint checking efficiently', () => {
      ...
      const { metrics } = PerformanceTestHelpers.assertPerformance(
        () => engine.applyVoiceLeading(longSequence1, longSequence2, complexConstraints),
        'mathematical-operations',
  ...
  describe('Comparative Performance', () => {
    it('should compare performance between species', async () => {
      ...
      expect(speciesComparison.comparison.function1.metrics.executionTime)
        .toBeLessThan(speciesComparison.comparison.function2.metrics.executionTime);
      expect(speciesComparison.comparison.significantDifference).toBe(true);
    });

    it('should compare voice leading algorithms', () => {
      const sourceNotes = Array.from({ length: 16 }, (_, i) => ({
        midi: 60 + i,
        velocity: 80,
        duration: 1,
      ...
      const { comparison } = PerformanceTestHelpers.comparePerformance(
  ...
  describe('Regression Performance Tests', () => {
    it('should maintain performance under repeated stress', async () => {
      ...
      expect(report.summary.p99Time).toBeLessThan(report.summary.meanTime * 3);
    ...
    it('should handle memory pressure gracefully', async () => {
      ...
      const promises = operations.map(op => PerformanceTestHelpers.assertPerformance(
        op,
        'mathematical-operations',
...
function createCantusFirmus(length: number, startMidi: number, endMidi: number): VoicePart {
  const notes = Array.from({ length }, (_, i) => {
    const midi = startMidi + Math.floor((i / (length - 1)) * (endMidi - startMidi));
    return {
      midi,
      velocity: 80,
      duration: 1,
