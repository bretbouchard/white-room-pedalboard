...
const CHORD_TYPES = [
  ...
  'major-7', 'minor-7', 'dominant-7', 'half-diminished-7',
  'diminished-7', 'augmented-7', 'major-6', 'minor-6'
];
...
const ORCHESTRAL_SECTIONS = ['strings', 'woodwinds', 'brass', 'percussion', 'keyboard'] as const;
...
const ORCHESTRAL_INSTRUMENTS: OrchestralInstrument[] = [
  // Strings
  { name: 'violin', section: 'strings', family: 'violin', range: [55, 103], transposition: 0, clef: 'treble' },
  { name: 'viola', section: 'strings', family: 'viola', range: [48, 91], transposition: 0, clef: 'alto' },
  { name: 'cello', section: 'strings', family: 'cello', range: [36, 79], transposition: 0, clef: 'bass' },
  { name: 'double-bass', section: 'strings', family: 'double-bass', range: [28, 67], transposition: -12, clef: 'bass' },

  // Woodwinds
  { name: 'flute', section: 'woodwinds', family: 'flute', range: [60, 98], transposition: 0, clef: 'treble' },
  { name: 'oboe', section: 'woodwinds', family: 'oboe', range: [58, 88], transposition: 0, clef: 'treble' },
  { name: 'clarinet', section: 'woodwinds', family: 'clarinet', range: [50, 92], transposition: -2, clef: 'treble' },
  { name: 'bassoon', section: 'woodwinds', family: 'bassoon', range: [34, 75], transposition: 0, clef: 'bass' },

  // Brass
  { name: 'trumpet', section: 'brass', family: 'trumpet', range: [52, 84], transposition: -2, clef: 'treble' },
  { name: 'horn', section: 'brass', family: 'horn', range: [34, 77], transposition: -7, clef: 'treble' },
  { name: 'trombone', section: 'brass', family: 'trombone', range: [34, 72], transposition: 0, clef: 'bass' },
  { name: 'tuba', section: 'brass', family: 'tuba', range: [22, 58], transposition: 0, clef: 'bass' },

  // Percussion
  { name: 'timpani', section: 'percussion', family: 'timpani', range: [36, 55], transposition: 0, clef: 'bass' },
  { name: 'xylophone', section: 'percussion', family: 'xylophone', range: [60, 96], transposition: 0, clef: 'treble' },

  // Keyboard
  { name: 'piano', section: 'keyboard', family: 'piano', range: [21, 108], transposition: 0, clef: 'treble' },
  { name: 'harpsichord', section: 'keyboard', family: 'harpsichord', range: [36, 96], transposition: 0, clef: 'treble' },
];
...
export const noteArbitrary: Arbitrary<Note> = fc.record({
  ...
  duration: fc.float({ min: DURATION_RANGE.min, max: DURATION_RANGE.max }),
 ...
 */
export const timeSignatureArbitrary: Arbitrary<TimeSignature> = fc.record({
  ...
  return commonDenominators.includes(ts.denominator) ||
 ...
 */
export const durationArbitrary = fc.float({
 ...
 */
export const rhythmPatternArbitrary: Arbitrary<RhythmPattern> = fc.record({
...
export const chordArbitrary: Arbitrary<Chord> = fc
  .record({
    ...
    extensions: fc.array(fc.constantFrom('7', '9', '11', '13', 'b5', '#5', 'b9', '#9'), { maxLength: 3 }),
  })
 ...
 */
export const musicalPhraseArbitrary: Arbitrary<MusicalPhrase> = fc.record({
  ...
  return phrase.notes.every(note =>
 ...
 */
export const melodyArbitrary = (length: { min?: number; max?: number } = { min: 4, max: 32 }) =>
  fc.array(noteArbitrary, { minLength: length.min, maxLength: length.max });

/**
 * Generator for chord progressions
 */
export const chordProgressionArbitrary = (length: { min?: number; max?: number } = { min: 2, max: 8 }) =>
  fc.array(chordArbitrary, { minLength: length.min, maxLength: length.max });

/**
 * Generator for rhythmic sequences
 */
export const rhythmSequenceArbitrary = (length: { min?: number; max?: number } = { min: 4, max: 16 }) =>
...
export const contrapuntalTextureArbitrary = (voices: number = 2) =>
  fc.tuple(...Array(voices).fill(null).map(() => melodyArbitrary({ min: 8, max: 32 })));

/**
 * Generator for orchestral combinations
 */
export const orchestralCombinationArbitrary = (minInstruments: number = 3, maxInstruments: number = 12) =>
  fc.array(orchestralInstrumentArbitrary, { minLength: minInstruments, maxLength: maxInstruments })
    .filter(instruments => {
...
export const tempoMarkingArbitrary = fc.record({
  ...
  term: fc.constantFrom(
    'grave', 'largo', 'adagio', 'andante', 'moderato',
...
export const articulationArbitrary = fc.constantFrom(
  'staccato', 'legato', 'accent', 'tenuto', 'fermata', 'trill', 'tremolo'
);
 ...
 */
export function generateChordStructure(root: number, type: string, inversion: number = 0): Chord {
  const basePattern = CHORD_PATTERNS[type] || CHORD_PATTERNS['major'];
  let notes = basePattern.map(interval => (root + interval) % 12);
