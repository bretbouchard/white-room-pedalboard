...
export class HardwareSimulator {
  ...
  async setEncoder(index: number, delta: number): Promise<void> {
    ...
    this.emit('encoderChanged', { index, delta, value: this.mockHardware.encoders[index] });
  }
...
export class HardwareTestHelpers {
  static createSimulator(config: Partial<HardwareConfig> = {}): HardwareSimulator {
  ...
  static async assertHardwareConnection(simulator: HardwareSimulator): Promise<void> {
    const connected = await simulator.connect();
    expect(connected).toBe(true);
    expect(simulator.getState().connected).toBe(true);
  }

  static async assertHardwareCalibration(simulator: HardwareSimulator): Promise<void> {
  ...
  static generateHardwareSequence(
    ...
    const promises = Array.from({ length: iterations }, (_, index) => operation(index));
...
export const HardwareTestDataGenerators = {
  ...
  randomFaderState: () => Array.from({ length: 8 }, () => Math.floor(Math.random() * 128)),
  randomKnobState: () => Array.from({ length: 8 }, () => Math.floor(Math.random() * 128)),
  randomButtonState: () => Array.from({ length: 16 }, () => Math.random() < 0.5),

  // Generate audio test signals
  generateSineWave: (frequency: number, amplitude: number, samples: number) =>
    Array.from({ length: samples }, (_, i) =>
      amplitude * Math.sin(2 * Math.PI * frequency * i / 44100)
    ),
