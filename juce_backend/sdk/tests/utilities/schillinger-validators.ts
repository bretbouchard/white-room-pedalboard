...
import type { Note, Interval, Scale, Chord, TimeSignature } from '../property-based/generators/musical-generators';
...
export class RhythmValidator {
   ...
   */
  static validateRhythmGrouping(notes: Note[], timeSignature: TimeSignature): ValidationResult {
    const errors: string[] = [];
    ...
    if (notes.length < SCHILLINGER_CONSTANTS.MIN_GROUP_SIZE) {
      warnings.push(`Rhythm group has fewer than ${SCHILLINGER_CONSTANTS.MIN_GROUP_SIZE} notes`);
    }

    if (notes.length > SCHILLINGER_CONSTANTS.MAX_GROUP_SIZE) {
      errors.push(`Rhythm group exceeds maximum of ${SCHILLINGER_CONSTANTS.MAX_GROUP_SIZE} notes`);
    }
  ...
  private static findCommonDivisor(durations: number[]): number {
    ...
    const commonNumerators = fractions.map(f => (f.numerator * lcm) / f.denominator);
    const gcd = commonNumerators.reduce((acc, n) => this.gcd(acc, Math.abs(n)), commonNumerators[0]);
   ...
   */
  private static calculateSyncopation(notes: Note[], timeSignature: TimeSignature): number {
    ...
    for (const note of notes) {
      ...
      for (const pos of beatPositions) {
        ...
        const isStrongBeat = beatInMeasure === 1 || (timeSignature.numerator % 2 === 0 && beatInMeasure === beatsPerMeasure / 2 + 1);
  ...
  private static calculateRegularity(durations: number[]): number {
    ...
    const averageDiff = differences.reduce((sum, d) => sum + d, 0) / differences.length;
    const variance = differences.reduce((sum, d) => sum + Math.pow(d - averageDiff, 2), 0) / differences.length;
...
export class HarmonyValidator {
  ...
  static validateChordProgression(chords: Chord[]): ValidationResult {
    ...
    for (let i = 1; i < chords.length; i++) {
      ...
      if (rootMovement > 7 && rootMovement < 12) {
        warnings.push(`Large root movement (${rootMovement} semitones) from chord ${i} to ${i + 1}`);
      }
      ...
      if (rootMovement === 7 || rootMovement === 5) {
        ...
        warnings.push(`Weak root movement (${rootMovement} semitones) from chord ${i} to ${i + 1}`);
      }
    }
   ...
   */
  private static analyzeVoiceLeading(chords: Chord[]): HarmonicAnalysis['voiceLeading'] {
...
export class PitchClassSetValidator {
  ...
  private static calculatePrimeForm(pcs: number[]): number[] {
    ...
    for (let transposition = 0; transposition < 12; transposition++) {
      const transposed = pcs.map(pc => (pc + transposition) % 12).sort((a, b) => a - b);
      const inverted = pcs.map(pc => (-pc + transposition) % 12).sort((a, b) => a - b);

      for (const form of [transposed, inverted]) {
        const span = this.calculateSpan(form);
        if (span < bestSpan || (span === bestSpan && this.isMoreCompact(form, bestForm))) {
          bestForm = form;
          bestSpan = span;
        }
      }
  ...
  private static calculateNormalOrder(pcs: number[]): number[] {
    ...
    for (let i = 0; i < pcs.length; i++) {
      ...
      if (span < bestSpan || (span === bestSpan && this.isMoreCompact(rotated, bestOrder))) {
        bestOrder = rotated;
        bestSpan = span;
      }
    }
...
export class MathPrecisionValidator {
   ...
   */
  static ratiosEqual(a: number, b: number, c: number, d: number, epsilon?: number): boolean {
  ...
  static findDenominator(value: number, maxDenominator: number = 1000): number {
    ...
    return -1; // No suitable denominator found
  }
}
...
export class SchillingerMathUtils {
  ...
  static harmonicTension(intervals: number[]): number {
    ...
    for (const interval of intervals) {
      ...
      if (intervalClass === 6) { // Tritone
        tension += 3;
      } else if (intervalClass === 2 || intervalClass === 10) { // Major second/minor seventh
        tension += 2;
      } else if (intervalClass === 1 || intervalClass === 11) { // Minor second/major seventh
        tension += 2.5;
      } else if ([3, 4, 8, 9].includes(intervalClass)) { // Thirds/sixths
        tension += 1;
      }
   ...
   */
  static voiceLeadingEfficiency(fromChord: number[], toChord: number[]): number {
    ...
    return totalMovement > 0 ? 1 / (1 + totalMovement / voices) : 1;
  }
