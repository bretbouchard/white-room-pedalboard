#include "websocket/SimpleWebSocketServer.h"
#include <iostream>
#include <thread>
#include <chrono>
#include <csignal>
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        if (port <= 0 || port > 65535) {
            std::cerr << "❌ Invalid port number: " << port << ". Using default port 8089." << std::endl;
            port = 8089;
        }
    }
    ...
    std::cout << "===============================" << std::endl;
    std::cout << "🔗 Testing simplified WebSocket implementation with proper handshake" << std::endl;

    // Create the simple WebSocket server
    ...
    std::cout << "🌐 Starting Simple WebSocket server on port " << port << "..." << std::endl;
    ...
    if (server.getTotalConnectionsHandled() > 0) {
        std::cout << "✅ SUCCESS: Simple WebSocket server accepted actual connections!" << std::endl;
    ...
    std::cout << "✅ Simple WebSocket server test completed successfully!" << std::endl;
