 ...
 */
export async function realtimePerformance(
  ...
  engine.onFrameUpdate((frame: RealizedFrame) => {
    console.log(
      `Frame: ${frame.time.seconds.toFixed(2)}s, Layers: ${frame.layers.length}, Coherence: ${frame.coherenceScore.toFixed(2)}`,
    );

    // Handle convergence events
    if (frame.convergenceFlags.approachingConvergence) {
      console.log(
        `Convergence approaching at ${frame.convergenceFlags.convergenceTime?.toFixed(2)}s (${frame.convergenceFlags.convergenceType})`,
      );
    }
  });
  ...
  engine.onConvergence((point) => {
    console.log(
      `Convergence detected: ${point.type} at ${point.time.toFixed(2)}s, strength: ${point.strength.toFixed(2)}`,
    );
  });
...
export function demonstrateEmergence(plane: RealizationPlane): void {
  ...
  for (let t = 0; t < 20; t += 2) {
    ...
    const highEmergence = emergenceScores.find(
      (score) => score.emergence > 0.7,
    );
    if (highEmergence) {
      ...
      releaseMoments: [
        {
          time: 15,
          type: "cascading",
          layers: ["texture", "ornament"],
          parameters: { cascadeDelay: 0.5 },
        },
        {
          time: 30,
          type: "staggered",
          layers: ["melody", "harmony", "bass"],
          parameters: { fadeTime: 3.0 },
        },
        {
          time: 45,
          type: "gradual",
