...
export class RealizationEngine implements IRealizationEngine {
  ...
  configure(config: Partial<RealizationEngineConfig>): void {
    ...
    if (
      config.bufferSize &&
      config.bufferSize !== this.frameBuffer["capacity"]
    ) {
  ...
  private startRealtimeMode(): void {
    const _targetFrameTime = 1000 / this.config.updateRate;
  ...
  private async startBatchMode(): Promise<void> {
    ...
    for (let frameIndex = 0; frameIndex < totalFrames; frameIndex++) {
      ...
      const musicalTime: RealizationTime = {
        seconds: frameTime,
      ...
      if (frameIndex % Math.floor(totalFrames / 10) === 0) {
        console.log(
          `Batch progress: ${Math.round((frameIndex / totalFrames) * 100)}%`,
        );
      }
    }
  ...
  private sendToRealtimeOutput(frame: RealizedFrame): void {
    ...
    if (Math.random() < 0.01) {
      ...
      console.log(
        `Realtime frame: ${frame.layers.length} layers, coherence: ${frame.coherenceScore.toFixed(2)}`,
      );
    }
  }
   ...
   */
  private storeForBatchExport(_frame: RealizedFrame): void {
   ...
   */
  private sendToStreamingOutput(_frame: RealizedFrame): void {
   ...
   */
  private getCurrentMusicalTime(): RealizationTime {
  ...
  private checkConvergences(frame: RealizedFrame): void {
    if (
      ...
      frame.convergenceFlags.convergenceTime
    ) {
  ...
  private countConvergences(): number {
    return this.frameHistory.filter(
      (frame) => frame.convergenceFlags.approachingConvergence,
    ).length;
  }
}
