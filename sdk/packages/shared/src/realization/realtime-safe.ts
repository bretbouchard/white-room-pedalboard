...
export class RealtimeFrameQueue {
  ...
  private copyFrameData(
    target: RealizedFrame,
    source: Partial<RealizedFrame>,
  ): void {
    ...
    target.convergenceFlags =
      source.convergenceFlags || target.convergenceFlags;
...
class RealizedFramePool {
  ...
  constructor(
    maxFrames: number,
    private maxEventsPerFrame: number,
  ) {
  ...
  private createEmptyFrame(_index: number): RealizedFrame {
    return {
...
export class EnvelopeGenerator {
  ...
  next(): number {
    switch (this.state) {
      ...
      case "decay":
        this.currentLevel +=
          (this.targetLevel - this.currentLevel) * this.decayRate;
