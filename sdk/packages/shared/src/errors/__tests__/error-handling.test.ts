...
describe("Error Handling - Comprehensive Edge Cases", () => {
  ...
  describe("Edge Cases and Boundary Conditions", () => {
    ...
    it("should handle memory pressure with many errors", () => {
      ...
      // Get baseline memory before creating errors
      if (typeof process !== "undefined" && process.memoryUsage) {
        ...
        // Allow up to 50MB growth from baseline for 1000 errors
        const memUsage = process.memoryUsage();
        const memGrowth = memUsage.heapUsed - baselineMem;
        expect(memGrowth).toBeLessThan(50 * 1024 * 1024); // Less than 50MB growth
      } else {
        // If process.memoryUsage is not available (browser environment),
