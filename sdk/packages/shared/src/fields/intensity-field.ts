...
import { IntensityField as IIntensityField } from "../types/realization";
...
export class IntensityField implements IIntensityField {
  ...
  private calculateCurveValue(time: number, duration: number): number {
    ...
    switch (this._curveType) {
      ...
      case "exponential-rise":
        return Math.max(
          0,
          Math.min(1, amplitude * Math.pow(normalizedTime, 2) + offset),
        );

      case "logarithmic-fall":
        return Math.max(
          0,
          Math.min(1, amplitude * (1 - Math.pow(normalizedTime, 0.5)) + offset),
        );
      ...
      case "sine-wave":
        return Math.max(
          ...
          Math.min(
            1,
            amplitude *
              Math.sin(2 * Math.PI * frequency * normalizedTime + phase) *
              0.5 +
              0.5 +
              offset,
          ),
      ...
      case "sawtooth": {
        const sawValue =
          (normalizedTime * frequency + phase / (2 * Math.PI)) % 1;
        return Math.max(0, Math.min(1, amplitude * sawValue + offset));
      }

      case "square": {
        const squareValue =
          Math.sin(2 * Math.PI * frequency * normalizedTime + phase) > 0
            ? 1
            : 0;
  ...
  private interpolate(time: number): number {
    ...
    let index = 0;
    while (
      index < this.timePoints.length - 1 &&
      this.timePoints[index + 1] <= time
    ) {
      index++;
    }
   ...
   */
  static blend(
    fields: Array<{ field: IntensityField; weight: number }>,
  ): IntensityField {
    if (fields.length === 0) {
    ...
    const blendedValues = referenceField.timePoints.map((time, _index) => {
  ...
  getStatistics(): {
    ...
    const variance =
      this.values.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) /
      this.values.length;
