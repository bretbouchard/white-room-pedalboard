...
export class CoincidenceField implements ICoincidenceField {
  ...
  hasConvergence(time: number, tolerance: number = 0.5): boolean {
    ...
    for (const point of this.convergencePoints) {
      if (
        ...
        point.strength >= this._sensitivity
      ) {
    ...
    for (const prediction of this._predictions) {
      if (
        Math.abs(prediction.time - time) <= tolerance &&
    ...
    const hasCurrentConvergence = this.analyzeCurrentConvergence(
      time,
      tolerance,
    );
  ...
  predictConvergence(currentTime: number): ConvergencePrediction[] {
    ...
    switch (this._algorithm) {
      case "phase-correlation":
        this._predictions = this.predictPhaseCorrelation(
          layers,
          currentTime,
          endTime,
        );
        break;
      case "peak-alignment":
        this._predictions = this.predictPeakAlignment(
          layers,
          currentTime,
          endTime,
        );
      ...
      case "energy-threshold":
        this._predictions = this.predictEnergyThreshold(
          layers,
          currentTime,
          endTime,
        );
      ...
      case "pattern-synchronization":
        this._predictions = this.predictPatternSynchronization(
          layers,
          currentTime,
          endTime,
        );
  ...
  getStatistics(): {
    ...
    const totalStrength = this.convergencePoints.reduce(
    ...
    if (this.convergencePoints.length > 1) {
      for (let i = 1; i < this.convergencePoints.length; i++) {
        totalInterval +=
          this.convergencePoints[i].time - this.convergencePoints[i - 1].time;
      }
   ...
   */
  private analyzeCurrentConvergence(time: number, _tolerance: number): boolean {
    const layers = Array.from(this._layers.values());
    ...
    const layerStates = layers.map((layer) => {
      const currentPhase =
        (layer.phase + layer.phaseVelocity * time) % (2 * Math.PI);
      return {
        ...layer,
        currentPhase,
        currentEnergy: layer.energy * Math.sin(currentPhase),
      };
    });
    ...
    for (let step = 0; step <= steps; step++) {
      ...
      if (phaseAlignment >= this._sensitivity) {
        ...
        const confidence = Math.min(
          1.0,
          (time - startTime) / this._lookaheadWindow,
        );
  ...
  private predictPeakAlignment(
    ...
    const layerPeaks = layers.map((layer) => {
      const peaks: number[] = [];
      const _period = (2 * Math.PI) / layer.frequency;
      let currentTime = startTime;

      while (currentTime <= endTime) {
        const phase =
          (layer.phase + layer.phaseVelocity * currentTime) % (2 * Math.PI);
        if (Math.abs(phase - Math.PI / 2) < 0.1) {
          // Near peak
    ...
    for (const peak1 of layerPeaks[0]?.peaks || []) {
      ...
      for (let i = 1; i < layerPeaks.length; i++) {
        const hasOverlap = layerPeaks[i].peaks.some(
          (peak2) => Math.abs(peak1 - peak2) < 0.2,
        );
        ...
        if (syncTime <= endTime) {
          const strength = this.calculatePhaseAlignment(
            layers.map((layer) => ({
              ...layer,
              phase:
                (layer.phase + layer.phaseVelocity * syncTime) % (2 * Math.PI),
            })),
          );
  ...
  private calculatePhaseAlignment(
    layers: Array<{ phase: number; energy?: number }>,
  ): number {
    ...
    const totalWeight = layers.reduce(
      (sum, layer) => sum + (layer.energy || 1),
      0,
    );
  ...
  private calculateEnergyAlignment(
    ...
    const meanEnergy =
      energyStates.reduce((sum, e) => sum + e, 0) / energyStates.length;

    // Calculate variance (lower variance = better alignment)
    const variance =
      energyStates.reduce((sum, e) => sum + Math.pow(e - meanEnergy, 2), 0) /
      energyStates.length;
  ...
  private predictConvergenceType(
    ...
    const hasMelody = layers.some(
      (l) => l.role === "melody" || l.role === "lead",
    );
    const hasHarmony = layers.some((l) => l.role === "harmony");
    const hasRhythm = layers.some(
      (l) => l.role === "rhythm" || l.role === "bass",
    );
   ...
   */
  private mergeNearbyPredictions(
    ...
    for (const prediction of sorted) {
      const existingIndex = merged.findIndex(
        (p) => Math.abs(p.time - prediction.time) <= tolerance,
      );

      if (existingIndex >= 0) {
        // Merge with existing prediction
        const existing = merged[existingIndex];
        existing.participatingLayers = [
          ...new Set([
            ...existing.participatingLayers,
            ...prediction.participatingLayers,
          ]),
        ];
