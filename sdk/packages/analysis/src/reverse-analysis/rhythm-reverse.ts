...
import type { RhythmPattern, RhythmicResultant } from "../types";

// Local implementation of generateRhythmicResultant
function generateRhythmicResultant(
  a: number,
  b: number,
  ...
  return {
    ...
    metadata: options?.includeMetadata
      ? {
          lcm,
          complexity: pattern.filter((x) => x > 0).length / pattern.length,
          density: pattern.reduce((sum, val) => sum + val, 0) / pattern.length,
        }
      : undefined,
  };
}
