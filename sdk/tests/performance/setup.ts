...
export class PerformanceMeasurement {
  ...
  private calculateStandardDeviation(): number {
    ...
    const variance =
      this.measurements.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) /
  ...
  getReport(): PerformanceReport {
    return {
      ...
      summary: {
        ...
        standardDeviation: this.calculateStandardDeviation(),
        maxMemoryIncrease:
          Math.max(...this.memorySnapshots.map((m) => m.heapUsed)) -
...
export class PerformanceTestHelpers {
  static async assertPerformance<T>(
    ...
    if (!report.summary.passed) {
      ...
      if (metrics.meanTime > config.maxExecutionTime) {
        failures.push(
          `Mean execution time ${metrics.meanTime.toFixed(2)}ms exceeds threshold ${config.maxExecutionTime}ms`,
        );
      }

      if (metrics.memoryIncrease > config.maxMemoryIncrease * 1024 * 1024) {
        failures.push(
          `Memory increase ${(metrics.memoryIncrease / 1024 / 1024).toFixed(2)}MB exceeds threshold ${config.maxMemoryIncrease}MB`,
        );
      }

      if (config.minThroughput && metrics.throughput < config.minThroughput) {
        failures.push(
          `Throughput ${metrics.throughput.toFixed(2)} ops/sec below threshold ${config.minThroughput} ops/sec`,
        );
      }
  ...
  static async measurePerformance<T>(
    ...
    const config = PerformanceThresholds[category];
    const measurement = new PerformanceMeasurement(config);
    return await measurement.measure(fn);
  }
  ...
  static comparePerformance<T>(
    ...
    return new Promise(async (resolve) => {
      ...
      const comparison: PerformanceComparison = {
        ...
        memoryRatio:
          metrics2.memoryIncrease / Math.max(metrics1.memoryIncrease, 1),
        winner: metrics1.meanTime < metrics2.meanTime ? "fn1" : "fn2",
        significantDifference:
          Math.abs(metrics1.meanTime - metrics2.meanTime) >
