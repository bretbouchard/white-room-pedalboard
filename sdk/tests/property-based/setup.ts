...
export const SchillingerArbitraries = {
  ...
  pattern: (length: number = 8) =>
    fc.array(fc.integer({ min: 0, max: 16 }), {
      minLength: length,
      maxLength: length,
    }),
  ...
  matrix: (rows: number, cols: number) =>
    fc.array(
      fc.array(fc.integer(-10, 10), { minLength: cols, maxLength: cols }),
  ...
  permutation: (length: number) =>
    fc
      .array(fc.integer({ min: 0, max: length - 1 }), {
        minLength: length,
        maxLength: length,
      })
...
export const PropertyTestHelpers = {
  // Test mathematical properties - use numeric arbitraries for math operations
  commutative: (operation: (a: number, b: number) => number) => {
    return fc.assert(
  ...
  associative: (operation: (a: number, b: number, c: number) => number) => {
    return fc.assert(
  ...
  identity: (operation: (a: number, b: number) => number, identity: number) => {
    return fc.assert(
      fc.property(fc.integer(), (a) => {
        const result1 = operation(a, identity);
        const result2 = operation(identity, a);
        return result1 === a && result2 === a;
      }),
    );
  },
};
...
export const PerformanceUtils = {
  ...
  assertPerformance: <T>(
    ...
    if (time > maxTimeMs) {
      throw new Error(
        `Performance assertion failed for ${description}: ${time}ms > ${maxTimeMs}ms`,
      );
    }
    return result;
  },
...
export const MemoryUtils = {
  ...
  assertMemoryLimit: (fn: () => void, maxIncreaseMB: number) => {
    ...
    if (increaseMB > maxIncreaseMB) {
      throw new Error(
        `Memory usage increased by ${increaseMB.toFixed(2)}MB, exceeding limit of ${maxIncreaseMB}MB`,
      );
    }
  },
};
...
export const TestDataGenerators = {
  ...
  harmonicExpansion: () => ({
    chord: fc.array(SchillingerArbitraries.note(), {
      minLength: 3,
      maxLength: 7,
    }),
  ...
  counterpoint: () => ({
    cantusFirmus: fc.array(SchillingerArbitraries.note(), {
      minLength: 8,
      maxLength: 16,
    }),
  ...
  contour: () => ({
    melody: fc.array(SchillingerArbitraries.note(), {
      minLength: 4,
      maxLength: 32,
    }),
    contourType: fc.constantFrom(
      "ascending",
      "descending",
      "arch",
      "v-shape",
