...
export class ScaleKnowledgeBase {
  ...
  private initializeScales(): void {
    ...
    for (const scale of scaleDefinitions) {
      ...
      for (const mode of scale.modes) {
        ...
        this.modeIndex.set(
          mode.name.toLowerCase().replace(/\s+/g, "_"),
          scale.id,
        );
      }
   ...
   */
  public async findByCharacteristics(
    characteristics: string[],
  ): Promise<Scale[]> {
    if (!this.initialized) {
      throw new Error("Scale knowledge base not initialized");
    ...
    for (const characteristic of characteristics) {
      const matchingScales = this.characteristicIndex.get(
   ...
   */
  private createScaleInstance(
    definition: SDKScaleDefinition,
    tonic: string,
  ): Scale {
