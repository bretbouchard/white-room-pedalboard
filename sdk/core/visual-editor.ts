...
export class VisualCompositionEditor extends EventEmitter {
  private _composition: VisualComposition;
  private _config: EditorConfiguration;
  ...
  private initializeConfiguration(
    userConfig?: Partial<EditorConfiguration>,
  ): EditorConfiguration {
  ...
  private handleMouseMove(event: MouseEvent): void {
    if (this.dragStart) {
      ...
      if (
        this._state.currentTool === "select" &&
  ...
  private pitchToY(pitch: number): number {
    ...
    const timelineHeight = 60;
    return (
      timelineHeight + (viewport.endPitch - pitch) * viewport.pixelsPerPitch
    );
  }
  ...
  public toggleRecording(): void {
    ...
    this.emit("recordingToggled", {
      recording: this._state.playbackState === "recording",
    });
  }
  ...
  public zoom(factor: number, centerX: number, centerY: number): void {
    ...
    this._viewport.pixelsPerBeat = Math.max(
      10,
      Math.min(200, this._viewport.pixelsPerBeat),
    );
  ...
  private startAutoSave(): void {
    setInterval(
      ...
      this._config.autoSaveInterval * 60 * 1000,
    );
  }
