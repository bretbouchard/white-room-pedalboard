...
import type { HarmonicAnalysis, ChordProgression, PatternIR_v1 } from "./ir";

// Re-export ChordProgression for use by other modules
export type { ChordProgression } from "./ir";
...
export class HarmonyAPI {
  ...
  async generateProgressionIR(
    ...
    if (options?.generators) {
      variationRules.push(
        `generators:${options.generators.a},${options.generators.b}`,
      );
    }
    ...
    if (options?.allowExtensions !== undefined) {
      variationRules.push(`extensions:${options.allowExtensions}`);
    }
    if (options?.allowAlterations !== undefined) {
  ...
  async generateChordIR(
    ...
    if (
      ...
      chordType.trim().length === 0
    ) {
      throw new _ValidationError(
        "chordType",
        chordType,
        "valid chord type string",
      );
    }
    if (!seed || typeof seed !== "string" || seed.trim().length === 0) {
      throw new _ValidationError("seed", seed, "non-empty string");
    ...
    // Validate inversion if provided
    if (options?.inversion !== undefined && options.inversion < 0) {
      throw new _ValidationError(
        "inversion",
        options.inversion,
    ...
    if (options?.inversion !== undefined) {
  ...
  async analyzeProgression(chords: string[]): Promise<HarmonicAnalysis> {
    ...
    if (this.sdk.isOfflineMode()) {
      ...
      offline.voice_leading_quality = roundFloat(offline.voice_leading_quality);
    ...
    const voice_leading_quality =
      ...
      (1 - (typeof vl.parallelMotion === "number" ? vl.parallelMotion : 0)) *
        0.1;
  ...
  private analyzeProgressionOffline(chords: string[]): HarmonicAnalysis {
    try {
      ...
      const subdominantCount = functions.filter(
        (f) => f === "subdominant",
      ).length;

      const key_stability =
        functions.length > 0
          ? (tonicCount * 0.6 + subdominantCount * 0.3 + dominantCount * 0.1) /
            functions.length
          : 0.5;

      // Calculate voice leading quality using interval analysis
      const voice_leading_quality =
        this.calculateVoiceLeadingQuality(chordAnalyses);

      // Generate meaningful suggestions based on analysis
      const suggestions = this.generateProgressionSuggestions(
        functions,
        tensionCurve,
        key_stability,
      );
      ...
      return {
        ...
        suggestions: [
          "Enhanced offline analysis failed, using basic fallback.",
        ],
      };
    }
  }
  ...
  private parseChordSymbol(chord: string): any {
    ...
    const extensionMatch = cleanChord.match(
      /(9|11|13|b9|#9|#11|b13|sus2|sus4)/g,
    );
    if (extensionMatch) {
  ...
  private calculateTensionCurve(
    ...
    return chordAnalyses.map((chord, index) => {
      ...
      if (chord.extensions.includes("b9") || chord.extensions.includes("#9"))
        tension += 0.2;
      if (chord.extensions.includes("#11") || chord.extensions.includes("b13"))
        tension += 0.1;
  ...
  private calculateVoiceLeadingQuality(chordAnalyses: any[]): number {
    ...
    for (let i = 1; i < chordAnalyses.length; i++) {
      ...
      const rootDistance = this.calculateRootDistance(
        prevChord.root,
        currChord.root,
      );
  ...
  private calculateRootDistance(root1: string, root2: string): number {
    ...
    const idx1 = chromaticScale.indexOf(
      root1.replace("#", "#").replace("b", "#"),
    );
    const idx2 = chromaticScale.indexOf(
      root2.replace("#", "#").replace("b", "#"),
    );
  ...
  private generateProgressionSuggestions(
    ...
    const tonicRatio =
      functions.filter((f) => f === "tonic").length / functions.length;
    const dominantRatio =
      functions.filter((f) => f === "dominant").length / functions.length;

    if (tonicRatio > 0.6) {
      suggestions.push(
        "Consider adding more dominant chords for forward motion.",
      );
    } else if (dominantRatio > 0.4) {
      suggestions.push(
        "Strong dominant presence - ensure proper resolutions to tonic.",
      );
    } else if (tonicRatio < 0.3) {
      suggestions.push(
        "Limited tonic stability - add more tonic chords for grounding.",
      );
    }
    ...
    if (tensionRange < 0.2) {
      suggestions.push(
        "Tension curve is relatively flat - consider more dynamic harmonic movement.",
      );
    } else if (maxTension > 0.8) {
      suggestions.push(
        "High tension detected - ensure strong resolutions for musical satisfaction.",
      );
    }
    ...
    if (keyStability > 0.7) {
      suggestions.push(
        "Very stable progression - suitable for sections requiring harmonic grounding.",
      );
    } else if (keyStability < 0.4) {
      suggestions.push(
        "Lower stability - consider where this progression fits in the larger musical structure.",
      );
    }
    ...
    if (functions.length === 2) {
      suggestions.push(
        "Two-chord progression - consider using for basic harmonic frameworks or pedal points.",
      );
    } else if (functions.length >= 8) {
      suggestions.push(
        "Extended progression - ensure clear phrases and cadential points.",
      );
    }

    return suggestions.length > 0
      ? suggestions
      : ["Progression appears well-balanced."];
  }

  private generateChordResolutions(
    chord: string,
    chordAnalysis: any,
    context: { key: string; scale: string },
  ): Array<{ target: string; type: string; probability: number }> {
    ...
    switch (chordFunction) {
      ...
      case "vi°":
        ...
        const subdominantChord = this.getSubdominant(
          context.key,
          context.scale,
        );
        resolutions.push({
    ...
    if (chordQuality.includes("dim") || chordQuality.includes("°")) {
      ...
      if (dominantResolution) {
        dominantResolution.probability = Math.min(
          1.0,
          dominantResolution.probability + 0.2,
        );
      }
    }

    // Normalize probabilities
    const totalProb = resolutions.reduce(
      (sum, res) => sum + res.probability,
      0,
    );
    if (totalProb > 0) {
      resolutions.forEach((res) => {
  ...
  async resolveChord(
    ...
    const chordAnalysis = analyzeChord(chord, context.key, context.scale);

    // Generate dynamic resolutions based on chord analysis
    const resolutions = this.generateChordResolutions(
      chord,
      chordAnalysis,
      context,
    );
  ...
  async encodeProgression(
    ...
    const encoding = encodeProgression(progression.chords, {
      maxGenerator: 16,
      minConfidence: 0.1,
      maxResults: 5,
  ...
  async findHarmonicMatches(
    ...
    const matches = inferHarmonicGenerators(targetProgression.chords, {
      maxGenerator: 16,
      minConfidence: 0.1,
      maxResults: 10,
      includeAlternatives: true,
  ...
  async analyzeVoiceLeadingAndRhythm(
    ...
    // Call with either full object or just chords array
    const analysis =
