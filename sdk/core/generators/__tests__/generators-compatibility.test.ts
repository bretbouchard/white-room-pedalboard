...
describe("Generator API Compatibility", () => {
  ...
  describe("RhythmGenerator Functionality", () => {
    it("should generate rhythmic resultants with enhanced metadata", async () => {
      const generator = new RhythmGenerator({ sdk });

      const result = await generator.generateResultant(3, 2);
  ...
  describe("HarmonyGenerator Functionality", () => {
    it("should generate chord progressions with enhanced metadata", async () => {
      const generator = new HarmonyGenerator({ sdk });

      const result = await generator.generateProgression({
        key: "C",
        scale: "major",
        length: 4,
      });
    ...
    it("should handle harmony variations", async () => {
      const generator = new HarmonyGenerator({ sdk });

      // Note: This test may need to be adjusted based on actual API implementation
      await expect(async () => {
        await generator.generateProgression({
          key: "C",
          scale: "major",
          length: 8,
          style: "jazz",
  ...
  describe("MelodyGenerator Functionality", () => {
    it("should generate melodic lines with enhanced metadata", async () => {
      const generator = new MelodyGenerator({ sdk });

      const result = await generator.generateMelody({
        key: "C",
        scale: "major",
        length: 8,
        contour: "ascending",
  ...
  describe("CompositionGenerator Functionality", () => {
    it("should create compositions with enhanced metadata", async () => {
  ...
  describe("Error Handling", () => {
    it("should handle operations without SDK gracefully", async () => {
      const generator = new RhythmGenerator(); // No SDK provided

      await expect(async () => {
        await generator.generateResultant(3, 2);
      }).rejects.toThrow("SDK instance required");
    });
