 ...
 */
export interface GeneratorMelodyGenerationParams {
  key: string;
  scale: string;
  length: number;
 ...
 */
export interface GeneratorMelodyVariationParams {
  type:
    | "inversion"
    | "retrograde"
    | "modal_shift"
...
> {
  ...
  async generateMelody(
    params: GeneratorMelodyGenerationParams,
  ): Promise<GeneratorResult<MelodyLine>> {
    ...
    try {
      ...
      // Delegate to existing functional API (method is generateLine, not generateMelody)
  ...
  ): Promise<GeneratorResult<MelodicAnalysis>> {
    const mergedParams = this.mergeParameters({
      key: melody.key,
      scale: melody.scale,
    });
  ...
  async generateVariations(
    melody: MelodyLine,
    params: GeneratorMelodyVariationParams,
  ): Promise<GeneratorResult<MelodyLine>> {
    const methodParams = {
      melodyId: melody.id,
      ...params,
    ...
    try {
      ...
      // Delegate to existing functional API
      const variation = await sdk.melody.generateVariations(
        melody,
        params.type,
        params,
      );
   ...
   * @param melody - The melody to analyze
   */
  async extractContour(melody: MelodyLine): Promise<GeneratorResult<string[]>> {
   ...
   */
  async findSimilarMelodies(
    contour: string[],
    params: { tolerance?: number; style?: string } = {},
  ): Promise<GeneratorResult<{ melody: MelodyLine; similarity: number }[]>> {
  ...
  completeMelody(
    ...
    try {
      ...
      const completedLength = Math.min(
        partialMelody.notes.length,
        targetLength,
      );
      const completedNotes = partialMelody.notes.slice(0, completedLength);
      const completedDurations = partialMelody.durations.slice(
        0,
        completedLength,
      );
      ...
      while (completedNotes.length < targetLength) {
        const lastNote =
          completedNotes[completedNotes.length - 1] ||
          this.parameters.range?.[0] ||
          60;
