import type { GeneratorResult, SchillingerSDK } from "@schillinger-sdk/shared";
...
export abstract class BaseGenerator<TConfig, TParams> {
  ...
  getSDK(): SchillingerSDK {
    if (!this.sdk) {
      throw new Error(
        `${this.generatorName}: SDK instance not available. Set SDK via constructor or setSDK() method.`,
      );
    }
    return this.sdk;
  }
  ...
  protected requireSDK(): SchillingerSDK {
    if (!this.sdk) {
      throw new Error(
        `${this.generatorName}: SDK instance required for this operation. Set SDK via constructor or setSDK() method.`,
      );
    }
    return this.sdk;
  }
...
export function isGenerator(obj: any): obj is BaseGenerator<any, any> {
  return (
    obj &&
    typeof obj === "object" &&
    "generatorName" in obj &&
    "setParameters" in obj
  );
}
