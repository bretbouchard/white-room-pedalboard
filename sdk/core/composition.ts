...
import type { PatternIR_v1, SongIR_v1 } from "./ir";
import {
  Composition,
  Section,
  SectionType,
...
export interface CompositionVariationParams {
  type: "rhythmic" | "harmonic" | "melodic" | "structural";
  intensity: "subtle" | "moderate" | "dramatic";
  sections?: SectionType[]; // which sections to vary
  preserveStructure?: boolean;
...
export class CompositionAPI {
  ...
  async generateCompositionIR(
    ...
    if (
      ...
      !Array.isArray(params.timeSignature) ||
      params.timeSignature.length !== 2
    ) {
      throw new _ValidationError(
        "timeSignature",
        params.timeSignature,
    ...
    try {
      ...
      try {
        ...
        encodeMusicalPattern =
          (mod as any).encodeMusicalPattern || (mod as any).default;
  ...
  private analyzeStructure(composition: Composition): StructuralAnalysis {
    ...
    for (let i = 0; i < composition.sections.length - 1; i++) {
      ...
      transitions.push({
        from: current.type as SectionType,
        to: next.type as SectionType,
    ...
    const sectionTypes = composition.sections.map(
      (s: Section) => s.type as SectionType,
    );
  ...
  private analyzeStructureOffline(
    ...
    const sectionTypes = composition.sections.map((s) => s.type as SectionType);
