...
import type { MelodyLine, RhythmPattern } from "@schillinger-sdk/shared";
import {
...
// Fugal development types
export interface FugalDevelopment {
  episodes: Array<{
    type: string;
    development: string;
    length: number;
...
export class CounterpointAPI {
   ...
   */
  async generateSpeciesCounterpoint(
    ...
    try {
      ...
      switch (species) {
        case 1:
          counterpoint = await this.generateFirstSpecies(
            cantusFirmus,
            key,
            mode,
            range,
          ...
          break;
        case 2:
          counterpoint = await this.generateSecondSpecies(
            cantusFirmus,
            key,
            mode,
            range,
          ...
          break;
        case 3:
          counterpoint = await this.generateThirdSpecies(
            cantusFirmus,
            key,
            mode,
            range,
          ...
          break;
        case 4:
          counterpoint = await this.generateFourthSpecies(
            cantusFirmus,
            key,
            mode,
            range,
          ...
          break;
        case 5:
          counterpoint = await this.generateFifthSpecies(
            cantusFirmus,
            key,
            mode,
            range,
          ...
          break;
        case 6:
          counterpoint = await this.generateSixthSpecies(
            cantusFirmus,
            key,
            mode,
            range,
      ...
      // Analyze the result
      const analysis = await this.analyzeCounterpoint(
        cantusFirmus,
        counterpoint,
        species,
      );
      ...
      if (
        error instanceof _ValidationError ||
        error instanceof _ProcessingError
      ) {
        throw error;
      }
  ...
  private async generateFirstSpecies(
    ...
    for (let i = 0; i < cantusFirmus.notes.length; i++) {
      ...
      const validNotes = consonantIntervals
        .map((intervalObj) => cfNote + intervalObj.interval)
      ...
      const bestNote = this.selectBestCounterpointNote(
        validNotes,
        counterpointNotes,
        cfNote,
      );
  ...
  private async generateSecondSpecies(
    ...
    for (let i = 0; i < cantusFirmus.notes.length; i++) {
      ...
      if (i === 0 || i === cantusFirmus.notes.length - 1) {
        ...
        const consonantNote = this.findConsonantNote(
          cfNote,
          scale,
          range,
          counterpointNotes,
        ...
        const firstNote = this.findConsonantNote(
          cfNote,
          scale,
          range,
          counterpointNotes,
        ...
        const secondNote = this.createPassingOrNeighborTone(
          firstNote,
          cfNote,
          i < cantusFirmus.notes.length - 1
            ? cantusFirmus.notes[i + 1]
            : cfNote,
          scale,
        );
    ...
    for (let i = 0; i < cantusFirmus.notes.length; i++) {
      ...
      if (i === 0 || i === cantusFirmus.notes.length - 1) {
        ...
        const notes = [
          ...
          this.createPassingOrNeighborTone(
            baseInterval.startNote,
            cfNote,
            cfNote,
            scale,
          ...
          baseInterval.endNote ||
            this.findConsonantNote(cfNote, scale, range, counterpointNotes),
        ];
  ...
  private async generateFourthSpecies(
    ...
    for (let i = 0; i < cantusFirmus.notes.length; i++) {
      ...
      if (i < cantusFirmus.notes.length - 1) {
        ...
        const currentConsonant = this.findConsonantNote(
          cfNote,
          scale,
          range,
          counterpointNotes,
        ...
        const suspensionNote = this.createSuspensionNote(
          currentConsonant,
          nextCfNote,
          scale,
        );
        ...
        const resolvingNote = this.findConsonantNote(
          cfNote,
          scale,
          range,
          counterpointNotes,
  ...
  private async generateFifthSpecies(
    ...
    for (let i = 0; i < cantusFirmus.notes.length; i++) {
      ...
      const technique = this.selectFifthSpeciesTechnique(
        i,
        cantusFirmus.notes.length,
      );
      const notes = await this.applyFifthSpeciesTechnique(
        cfNote,
        cfDuration,
        scale,
        range,
   ...
   */
  async generateFugalImitation(
    subject: MelodyLine,
    options: FugalOptions,
  ): Promise<FugalResult> {
  ...
  private async createImitativeVoice(
    ...
    const displacedNotes = Array(timeDisplacement)
      .fill(0)
      .concat(transposedNotes);
  ...
  async generateCanon(
    ...
    try {
      ...
      const analysis = await this.analyzeCanon(
        melody,
        canonVoices,
        options.type,
      );
  ...
  private async createIntervalCanon(
    melody: MelodyLine,
    options: CanonOptions,
  ): Promise<MelodyLine[]> {
  ...
  private async createRetrogradeCanon(
    melody: MelodyLine,
    options: CanonOptions,
  ): Promise<MelodyLine[]> {
    const voices: MelodyLine[] = [melody];
  ...
  private async createInversionCanon(
    melody: MelodyLine,
    options: CanonOptions,
  ): Promise<MelodyLine[]> {
    const voices: MelodyLine[] = [melody];
    ...
    for (let i = 1; i < voiceCount; i++) {
      const invertedNotes = melody.notes.map((note) =>
        this.invertInterval(note, melody.notes[0]),
      );
      voices.push({
  ...
  private createVoiceLeadingProblem(
    voices: MelodyLine[],
    constraints: VoiceLeadingConstraints,
  ) {
    return {
  ...
  private async solveVoiceLeadingOptimization(
    problem: any,
  ): Promise<{ voices: MelodyLine[]; score: number }> {
  ...
  private async analyzeCounterpoint(
    ...
    const voiceLeadingQuality = this.calculateVoiceLeadingQuality(
      cantusFirmus,
      counterpoint,
    );
    const consonanceRatio = this.calculateConsonanceRatio(
      cantusFirmus,
      counterpoint,
    );
    const dissonanceResolution = this.findDissonanceResolutions(
      cantusFirmus,
      counterpoint,
    );
    const parallelIntervals = this.findParallelIntervals(
      cantusFirmus,
      counterpoint,
    );
  ...
  private async analyzeFugalImitation(
    subject: MelodyLine,
    voices: MelodyLine[],
  ): Promise<any> {
    return {
  ...
  private async analyzeCanon(
    melody: MelodyLine,
    canonVoices: MelodyLine[],
    canonType: string,
  ): Promise<any> {
    return {
      structuralCoherence: this.calculateStructuralCoherence(
        melody,
        canonVoices,
      ),
      intervalRelationships: this.analyzeIntervalRelationships(
        melody,
        canonVoices,
      ),
  ...
  private validateSpeciesOptions(options: SpeciesOptions): void {
    ...
    if (options.species < 1 || options.species > 6) {
      throw new _ValidationError(
        "species",
        options.species,
        "species between 1 and 6",
      );
    }
    if (
      !Array.isArray(options.cantusFirmus.notes) ||
      options.cantusFirmus.notes.length === 0
    ) {
      throw new _ValidationError(
        "cantusFirmus.notes",
        options.cantusFirmus.notes,
        "non-empty array of notes",
      );
    }
  }

  private validateFugalOptions(
    subject: MelodyLine,
    options: FugalOptions,
  ): void {
    if (!subject) {
  ...
  private getConsonantIntervals(
    pitchClass: number,
  ): { interval: number; startNote: number; endNote?: number }[] {
    // Perfect consonances (unison, octave, perfect fifth, perfect fourth)
    const perfectConsonances = [
      { interval: 0, startNote: pitchClass, endNote: pitchClass }, // Unison
      { interval: 12, startNote: pitchClass, endNote: pitchClass + 12 }, // Octave
      { interval: 7, startNote: pitchClass, endNote: pitchClass + 7 }, // Perfect fifth
      { interval: 5, startNote: pitchClass, endNote: pitchClass + 5 }, // Perfect fourth
    ];

    // Imperfect consonances (major/minor thirds, major/minor sixths)
    const imperfectConsonances = [
      { interval: 4, startNote: pitchClass, endNote: pitchClass + 4 }, // Major third
      { interval: 3, startNote: pitchClass, endNote: pitchClass + 3 }, // Minor third
      { interval: 9, startNote: pitchClass, endNote: pitchClass + 9 }, // Major sixth
      { interval: 8, startNote: pitchClass, endNote: pitchClass + 8 }, // Minor sixth
    ];
  ...
  private findConsonantNote(
    ...
    for (const interval of consonantIntervals) {
      ...
      if (
        candidateNote >= range[0] &&
        candidateNote <= range[1] &&
        this.isNoteInScale(candidateNote, scale)
      ) {
        // Avoid repetition when possible
        if (
          existingNotes.length === 0 ||
          candidateNote !== existingNotes[existingNotes.length - 1]
        ) {
    ...
    const lastInterval =
      existingNotes.length > 1
        ? existingNotes[existingNotes.length - 1] -
          existingNotes[existingNotes.length - 2]
        : 0;
    const contraryCandidates = validNotes.filter(
      (note) => Math.abs((note - cfNote) * lastInterval) < 0,
    );

    return contraryCandidates.length > 0
      ? contraryCandidates[0]
      : validNotes[0];
  }
  ...
  private createPassingOrNeighborTone(
    ...
    if (
      this.isNoteInScale(candidate, scale) &&
  ...
  private createSuspensionNote(
    preparationNote: number,
  ...
  private findConsonantInterval(cfNote: number): {
    startNote: number;
    endNote?: number;
  } {
  ...
  private selectFifthSpeciesTechnique(
    position: number,
    totalLength: number,
  ): string {
    const techniques = [
  ...
  private async applyFifthSpeciesTechnique(
    ...
    switch (technique) {
      case "first_species":
        const note = this.findConsonantNote(
          cfNote,
          scale,
          range,
          existingNotes,
      ...
      case "second_species":
        ...
        return {
          ...
          durations: [cfDuration / 2, cfDuration / 2],
        };
      ...
      case "third_species":
        ...
        return {
          notes: [
            ...
            baseInterval.endNote ||
              this.findConsonantNote(cfNote, scale, range, existingNotes),
          ],
          durations: [
            cfDuration / 4,
            cfDuration / 4,
      ...
      case "florid_ornament":
        ...
        const baseNote = this.findConsonantNote(
          cfNote,
          scale,
          range,
          existingNotes,
        ...
        return {
          ...
          durations: [
            cfDuration * 0.6,
            ...ornaments.map(() => (cfDuration * 0.4) / ornaments.length),
          ],
        };

      case "combination":
        // Combine multiple techniques
        return this.applyFifthSpeciesTechnique(
          cfNote,
          cfDuration,
          scale,
          range,
  ...
  private calculateVoiceLeadingQuality(
    cantusFirmus: MelodyLine,
    counterpoint: MelodyLine,
  ): number {
  ...
  private calculateConsonanceRatio(
    cantusFirmus: MelodyLine,
    counterpoint: MelodyLine,
  ): number {
    const consonantCount = this.countConsonantIntervals(
      cantusFirmus,
      counterpoint,
    );
    const totalIntervals = Math.min(
      cantusFirmus.notes.length,
      counterpoint.notes.length,
    );
    return consonantCount / totalIntervals;
  }

  private countConsonantIntervals(
    melody1: MelodyLine,
    melody2: MelodyLine,
  ): number {
  ...
  private findDissonanceResolutions(
    cantusFirmus: MelodyLine,
    counterpoint: MelodyLine,
  ): number[] {
    ...
    for (let i = 1; i < minLength; i++) {
      ...
      if (!this.isConsonantInterval(interval)) {
        ...
        if (i < minLength - 1) {
          const nextInterval = Math.abs(
            counterpoint.notes[i + 1] - cantusFirmus.notes[i + 1],
          );
  ...
  private findParallelIntervals(
    melody1: MelodyLine,
    melody2: MelodyLine,
  ): Record<
    number,
    Array<{ measure: number; beat: number; voices: [number, number] }>
  > {
    const parallels: Record<
      number,
      Array<{ measure: number; beat: number; voices: [number, number] }>
    > = {};
  ...
  private analyzeLeaps(melody: MelodyLine): {
    ...
    const averageLeap =
      totalLeaps > 0
        ? leaps.reduce((sum, interval) => sum + Math.abs(interval), 0) /
          totalLeaps
        : 0;
  ...
  private calculateImitationAccuracy(
    subject: MelodyLine,
    voices: MelodyLine[],
  ): number {
    let totalAccuracy = 0;
    ...
    for (let i = 1; i < voices.length; i++) {
      ...
      const subjectNotes = subject.notes.slice(
        0,
        subject.notes.length - startIndex,
      );
      ...
      for (let j = 0; j < minLength; j++) {
        const transposedSubjectNote =
          subjectNotes[j] + (voiceNotes[0] - subjectNotes[0]);
  ...
  private calculateVoiceIndependence(voices: MelodyLine[]): number {
    ...
    for (let i = 0; i < voices.length; i++) {
      for (let j = i + 1; j < voices.length; j++) {
        const independence = this.calculatePairwiseIndependence(
          voices[i],
          voices[j],
        );
  ...
  private calculatePairwiseIndependence(
    voice1: MelodyLine,
    voice2: MelodyLine,
  ): number {
  ...
  private analyzeHarmonicStructure(voices: MelodyLine[]): number[] {
    ...
    for (let i = 0; i < voices.length; i++) {
      for (let j = i + 1; j < voices.length; j++) {
        const minLength = Math.min(
          voices[i].notes.length,
          voices[j].notes.length,
        );
  ...
  private assessDevelopmentPotential(
    ...
    const developmentPotential =
      subjectFeatures.rhythmicInterest * 0.3 +
  ...
  private analyzeSubjectFeatures(subject: MelodyLine): any {
    ...
    return {
      rhythmicInterest: this.calculateRhythmicVariety(
        subject.durations || subject.notes.map(() => 1),
      ),
  ...
  private calculateMelodicSimilarity(
    melody1: MelodyLine,
    melody2: MelodyLine,
  ): number {
  ...
  private analyzeIntervalRelationships(
    original: MelodyLine,
    canonVoices: MelodyLine[],
  ): number[] {
    ...
    for (const voice of canonVoices) {
      ...
      if (alignmentIndex !== -1) {
        ...
        const alignedLength = Math.min(
          ...
          voice.notes.length - alignmentIndex,
        );
        let matches = 0;
        ...
        for (let i = 0; i < alignedLength; i++) {
          ...
          const transposedVoice =
            voiceNote - voice.notes[0] + original.notes[0];
  ...
  private countParallelIntervals(
    voices: MelodyLine[],
    disallowed: number[],
  ...
  private createFugalDevelopment(
    subject: MelodyLine,
    voices: MelodyLine[],
  ): {
    episodes: any[];
