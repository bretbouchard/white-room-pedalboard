...
export class ErrorHandler {
  ...
  static async handleError(
    ...
    const schillingerError =
      error !== undefined && "code" in error && "category" in error
        ? (error as SchillingerError)
    ...
    const userImpact = (
      this.constructor as typeof ErrorHandler
    ).assessUserImpact(schillingerError, recoveryOptions?.userLevel);

    // Create error report
    const report: ErrorReport = {
      id: (this.constructor as typeof ErrorHandler).generateId(),
      error: schillingerError,
    ...
    if (
      recoveryOptions?.attemptRecovery !== false &&
      schillingerError.recoverable
    ) {
      const recoveryResult = await handler.attemptRecovery(
        schillingerError,
        report,
      );
      ...
      if (recoveryResult.success) {
        ...
        report.recoveryAttempts.push({
          ...
          success: true,
          result: recoveryResult.result || (recoveryResult as RecoveryResult),
  ...
  private static generateUserFriendlyMessage(error: SchillingerError): string {
    const messageTemplates = {
      ...
      [ErrorCategory.RESOURCE]: {
        ...
        [ErrorSeverity.HIGH]: "Resource exhaustion",
        [ErrorSeverity.CRITICAL]: "Critical resource failure",
      },
      [ErrorCategory.EXTERNAL]: {
        [ErrorSeverity.LOW]: "External service issue",
        [ErrorSeverity.MEDIUM]: "External service error",
        [ErrorSeverity.HIGH]: "External service failure",
        [ErrorSeverity.CRITICAL]: "Critical external service error",
      },
      [ErrorCategory.SYSTEM]: {
        [ErrorSeverity.LOW]: "System issue detected",
        [ErrorSeverity.MEDIUM]: "System error occurred",
        [ErrorSeverity.HIGH]: "System failure",
        [ErrorSeverity.CRITICAL]: "Critical system error",
      },
      [ErrorCategory.USER_INPUT]: {
        [ErrorSeverity.LOW]: "Input issue",
        [ErrorSeverity.MEDIUM]: "Invalid input",
        [ErrorSeverity.HIGH]: "Bad input format",
        [ErrorSeverity.CRITICAL]: "Fatal input error",
      },
    };

    return (
      messageTemplates[error.category as keyof typeof messageTemplates]?.[
        error.severity as keyof (typeof messageTemplates)[typeof error.category]
  ...
  private static generateTechnicalDetails(error: SchillingerError): string {
    ...
    if (
      ...
      Object.keys(error.context.parameters).length > 0
    ) {
      details.push(
        `Parameters: ${JSON.stringify(error.context.parameters, null, 2)}`,
      );
    }
  ...
  private static assessUserImpact(
    ...
    if (error.category === ErrorCategory.VALIDATION && userLevel === "beginner")
      return "major";
    if (error.severity === ErrorSeverity.HIGH && userLevel === "beginner")
      return "major";
  ...
  private async attemptRecovery(
    ...
    for (const strategy of strategies) {
      try {
        ...
        console.warn(
          `Recovery strategy failed: ${(recoveryError as Error).message}`,
        );
      }
    }
   ...
   */
  registerStrategies(
    strategies: Array<{ name: string; strategy: RecoveryStrategy }>,
  ): void {
  ...
  private updateCommonPatterns(error: SchillingerError): void {
    ...
    const existing = this.statistics.commonPatterns.find(
      (p) => p.pattern === pattern,
    );
    ...
    if (existing) {
      ...
      this.statistics.commonPatterns.push({
        ...
        suggestedFix:
          error.suggestions[0]?.description || "Refer to documentation",
      });
    }
    ...
    if (this.statistics.commonPatterns.length > 10) {
      this.statistics.commonPatterns = this.statistics.commonPatterns.slice(
        0,
        10,
      );
    }
  ...
  getRecentErrors(limit: number = 50): ErrorReport[] {
    const sorted = Array.from(this.errorLog.values()).sort(
  ...
  private registerDefaultStrategies(): void {
    ...
    this.registerStrategy("validation_suggest_alternatives", {
      ...
      description:
        "Suggest alternative input values when fixing isn't possible",
      ...
      execute: async (error, context) => {
        ...
        } as RecoveryResult & { suggestions?: typeof error.suggestions };
      },
    });
    ...
    this.registerStrategy("engine_retry_operation", {
      ...
      applicability: (error) =>
        error.category === ErrorCategory.ENGINE &&
    ...
    this.registerStrategy("pipeline_skip_stage", {
      ...
      applicability: (error) =>
        error.category === ErrorCategory.PIPELINE &&
        error.severity <= ErrorSeverity.MEDIUM,
      execute: async (error, context) => {
        return {
          success: true,
...
export function createValidationError(
  ...
  return createError(
    ...
    ErrorSeverity.MEDIUM,
    {
      ...
      parameters: { parameter, value, expectedType },
    },
...
/**
 * Safe execute wrapper that never throws
 */
...
/**
 * Fallback values for various operations
 */
