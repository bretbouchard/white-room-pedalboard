...
import type { RhythmPattern } from "./ir";
...
const inferGenerators = (pattern: any, options: any) => {
  ...
  const patternGcd = durations.reduce(
    (acc: number, val: number) => gcd(acc, val),
    durations[0],
  );
  ...
  const maxGenerator = Math.min(
    options.maxGenerator || 16,
    Math.max(...durations),
  );
  ...
  return candidates.length > 0
    ? candidates
    : [{ generators: { a: 4, b: 3 }, confidence: 0.3 }];
};
...
const findBestFit = (pattern: any, options: any) => {
  ...
  return inferences.map((inf) => {
    ...
    const resultant = generateResultantPattern(
      inf.generators.a,
      inf.generators.b,
    );
    return {
...
const reverseAnalyzeComplex = (pattern: any, options: any) => {
  try {
    ...
    const isPolyrhythmic =
      primaryInferences.length > 1 &&
      Math.abs(
        primaryInferences[0].confidence - primaryInferences[1].confidence,
      ) < 0.2;

    const complexityScore = calculateOverallComplexity(
      pattern,
      primaryInferences,
    );
...
function findSecondaryGenerators(
  pattern: any,
  options: any,
): Array<{ generators: { a: number; b: number }; confidence: number }> {
  ...
  return inferGenerators(
    { durations: subdivisions },
    {
      ...options,
      maxGenerator: 8,
    },
...
function calculateOverallComplexity(pattern: any, inferences: any[]): number {
  const baseComplexity = pattern.durations
    ? MathUtils.calculateComplexity(pattern.durations)
    : 0.5;

  const inferenceComplexity =
    inferences.length > 0 ? 1 - inferences[0].confidence : 0.5; // Lower confidence = higher complexity
...
export class RhythmAPI {
  ...
  async generateResultantIR(
    ...
    // Return IR descriptor (no actual generation yet)
    // The pattern can be deterministically generated from this IR
    return {
      version: "1.0",
...
// Re-export Rational from the rational module
