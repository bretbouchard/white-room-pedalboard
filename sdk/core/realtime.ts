...
export class RealtimeManager {
  ...
  private static ConnectionGuards = class ConnectionGuards {
    ...
    basicRejectGuard(ctx: { mySeq?: number }): boolean {
      ...
      if (
        typeof this.parent._lastSuccessfulConnectSeq !== "undefined" &&
        this.parent._lastSuccessfulConnectSeq === mySeq
      )
        return true;
    ...
    advancedRejectGuard(ctx: {
      ...
      myWsInstance?: any;
      expectedReject?: ((e?: any) => void) | undefined;
    }): boolean {
      const {
        mySeq = -1,
        myDisconnectMark = -1,
        connectToken = undefined,
        myWsInstance = undefined,
      ...
      try {
        if (
          this.parent.connectionState &&
          (this.parent.connectionState.status === "connected" ||
            this.parent.connectionState.status === "disconnected")
        )
          return true;
      } catch (_) {}
      try {
        ...
        const OPEN =
          ...
          (global as any).WebSocket.OPEN !== undefined
      ...
      if (expectedReject && this.parent._connectReject !== expectedReject)
        return true;
      return false;
    }
  };
  ...
  private static ConnectionController = class ConnectionController {
    ...
    atomicallyReject(
      ...
      if (
        this.guards.advancedRejectGuard({
          mySeq,
          myDisconnectMark,
          connectToken,
          myWsInstance,
      ...
      try {
        this.parent._connectReject = undefined;
        this.parent._connectResolve = undefined;
      } catch (_) {}
      try {
        if (this.parent._connectDeferredTimeout) {
          clearTimeout(this.parent._connectDeferredTimeout);
          this.parent._connectDeferredTimeout = undefined;
        }
      } catch (_) {}
      try {
      ...
      try {
        if (this.parent._connectTimeout) {
          ...
          this.parent._connectTimeout = undefined;
        }
      } catch (_) {}
      ...
      try {
        ...
        try {
          console.log(
            "[ConnectionController.atomicallyReject] stored reject threw",
            e,
          );
        } catch (_) {}
      }
    ...
    handleConnectTimeout(
      ...
      return (this.parent as any)._handleConnectTimeout(
        mySeq,
        myDisconnectMark,
        connectToken,
        myAttemptCtrl,
    ...
    runDeferredConnectReject(
      ...
      return (this.parent as any)._runDeferredConnectReject(
        mySeq,
        myDisconnectMark,
        connectToken,
        _localRejectRef,
  ...
  private handleMessage = (ev: any) => {
    try {
      ...
      safeExecute("RealtimeManager.handleMessage.debug", () =>
        console.debug(
          "[RealtimeManager.handleMessage] parsedType=",
          parsed && parsed.type,
        ),
      );
  ...
  private _parseMessage(ev: any): any | undefined {
    try {
      ...
      try {
        console.debug(
          "[RealtimeManager._parseMessage] raw=",
  ...
  private _handleConnectOpen(ctx: {
    mySeq: number;
    myAttemptCtrl: any;
    timeout: any;
    setSettled: (v: boolean) => void;
    ...
    try {
      ...
      try {
        if (this._connectDeferredTimeout) {
          ...
          this._connectDeferredTimeout = undefined;
        }
      } catch (_) {}
      try {
      ...
      try {
        console.log(
          "[RealtimeManager.onOpen] readyState=",
          this.ws?.readyState,
        );
      } catch (_) {}
      try {
        (this.ws as any).__connected = true;
      } catch (_) {}
      const prevAttempts =
      ...
      try {
        ...
        try {
          this._currentConnectAbort = undefined;
        } catch (_) {}
        ...
        console.warn(
          "[ConnectionController._atomicallyReject] Error during cleanup:",
  ...
  private _handleConnectClose(ctx: {
    ev?: any;
    mySeq: number;
    timeout: any;
    getSettled: () => boolean;
    ...
    try {
      const { ev, mySeq, timeout, getSettled, setSettled, atomicallyReject } =
        ctx;
      ...
      this.stopHeartbeat();
      const wasConnected =
        this.connectionState && this.connectionState.status === "connected";
      const code =
        ev && typeof ev === "object" && "code" in ev ? ev.code : undefined;
      const normalClose =
        code === 1000 ||
        (code === undefined && !wasConnected) ||
        this._manualClose;
      if (wasConnected && !normalClose) {
        ...
        if (nextAttempts <= ((this.options as any).maxReconnectAttempts || 5)) {
          ...
          this.connectionState = {
            status: "error",
            reconnectAttempts: nextAttempts,
            error: new NetworkError("Max reconnect attempts exceeded"),
          } as any;
          this.emit("connectionStateChanged", this.connectionState);
        }
        if (!getSettled() && mySeq === this._connectSeq) {
          ...
          try {
            ...
            if (didReject) {
              ...
              try {
                console.debug(
                  "[RealtimeManager.onClose] atomicallyReject returned false; likely manual disconnect",
                );
              } catch (_) {}
            }
          } catch (_) {}
        ...
        if (!getSettled() && mySeq === this._connectSeq && !normalClose) {
          if (
            mySeq !== this._connectSeq ||
            this._manualClose ||
            this._connectSettled
          ) {
            return;
          }
          setSettled(true);
          ...
          if (
            !atomicallyReject(new NetworkError("WebSocket connection error"))
          ) {
            return;
          }
        }
  ...
  private _handleConnectError(ctx: {
    e?: any;
    mySeq: number;
    timeout: any;
    atomicallyReject: (
    ...
    try {
      const { e, mySeq, timeout, atomicallyReject, getSettled, setSettled } =
        ctx;
      if (getSettled() || mySeq !== this._connectSeq) return;
      setSettled(true);
      try {
      ...
      try {
        ...
        if (didReject) {
          ...
          try {
            console.debug(
              "[RealtimeManager.onError] atomicallyReject returned false; skipping connectionState update",
            );
          } catch (_) {}
        }
      } catch (_) {}
    ...
    try {
      ...
      try {
        if (getSettled()) {
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] local settled=true, ignoring",
            );
          } catch (_) {}
          return;
        }
        if (this.ws == null) {
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] ws is null, ignoring",
            );
          } catch (_) {}
          return;
        }
        if (this._disconnectMarker !== myDisconnectMark) {
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] disconnect marker changed, ignoring",
            );
          } catch (_) {}
          return;
        }
        if (this._suppressConnectRejection) {
          ...
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] suppressed due to explicit disconnect (early)",
            );
          } catch (_) {}
          try {
        ...
        if (!this._connectTimeout) {
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] no _connectTimeout, ignoring",
            );
          } catch (_) {}
          return;
        }
        if (this._connectToken !== connectToken) {
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] token mismatch, ignoring",
            );
          } catch (_) {}
          return;
        }
        if (mySeq !== this._connectSeq) {
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] mySeq != currentSeq, ignoring",
              { mySeq, currentSeq: this._connectSeq },
            );
          } catch (_) {}
          return;
        }
        if (this._manualClose || this._connectSettled) {
          ...
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] manualClose or already settled, ignoring",
            );
          } catch (_) {}
          return;
        }
      } catch (_) {
        /* continue to deeper checks if diagnostics fail */
      }

      const currentReady = this.ws?.readyState;
      try {
        console.debug(
          "[RealtimeManager.connect.timeout] mySeq=",
          mySeq,
          "currentSeq=",
          this._connectSeq,
          "manual=",
      ...
      if (currentReady !== OPEN && currentReady !== (WebSocket as any).OPEN) {
        if (
          ...
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] re-check guard triggered, aborting reject",
            );
          } catch (_) {}
          return;
        }
        try {
          const s =
            this.connectionState && (this.connectionState.status as string);
          if (s === "connected" || s === "disconnected") {
            ...
            try {
              console.debug(
                "[RealtimeManager.connect.timeout] connectionState already connected/disconnected, ignoring",
              );
            } catch (_) {}
            return;
          }
        ...
        try {
          console.error("[RealtimeManager.connect.timeout] rejecting: state=", {
            mySeq,
            currentSeq: this._connectSeq,
            manualClose: this._manualClose,
            connectSettled: this._connectSettled,
        ...
        if (this._manualClose) {
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] manualClose detected at final check, aborting reject",
            );
          } catch (_) {}
          return;
        }
        if (this._suppressConnectRejection) {
          ...
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] suppressed reject due to explicit disconnect",
            );
          } catch (_) {}
          return;
        }
        if (
          ...
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] connectionState is disconnected, aborting reject",
            );
          } catch (_) {}
          return;
        }
        ...
        try {
          ...
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] scheduling deferred reject timer (grace)",
            );
          } catch (_) {}
          ...
          this._connectDeferredTimeout = setTimeout(() => {
            try {
              if (
                ...
                typeof this._connectionController.runDeferredConnectReject ===
                  "function"
              ) {
                this._connectionController.runDeferredConnectReject(
                  mySeq,
                  myDisconnectMark,
                  connectToken,
                  _localRejectRef,
                ...
                this._runDeferredConnectReject(
                  mySeq,
                  myDisconnectMark,
                  connectToken,
                  _localRejectRef,
          ...
          try {
            console.debug(
              "[RealtimeManager.connect.timeout] scheduling deferred reject failed",
            );
          } catch (_) {}
        }
      }
  ...
  private _atomicallyReject(
    ...
    try {
      ...
      try {
        ...
        try {
          console.log(
            "[RealtimeManager._atomicallyReject] stored reject threw",
            e,
          );
        } catch (_) {}
      }
  ...
  private _createWebSocketInstance(tokenArg?: string): any {
    ...
    const effectiveUrl = tokenArg
      ? baseUrl +
        (baseUrl.includes("?") ? "&" : "?") +
        `token=${encodeURIComponent(tokenArg)}`
      : baseUrl;
    ...
    if (injected) {
      const looksLikeConstructor = !!(
        injected &&
        injected.prototype &&
        typeof injected.prototype.send === "function"
      );
    ...
    if (globalMock && typeof globalMock === "object") {
      try {
        ...
        const CLOSED =
          (global as any).WebSocket &&
          (global as any).WebSocket.CLOSED !== undefined
            ? (global as any).WebSocket.CLOSED
            : 3;
        const CLOSING =
          (global as any).WebSocket &&
          (global as any).WebSocket.CLOSING !== undefined
            ? (global as any).WebSocket.CLOSING
            : 2;
        if (rs !== CLOSED && rs !== CLOSING) {
          inst = globalMock;
          try {
            (inst as any).__claimedBy = this;
          } catch (_) {}
          try {
    ...
    if (!inst) {
      if (typeof WS === "function") {
        try {
          inst = (WS as any)(effectiveUrl, this.options.protocols);
        } catch (_) {}
      }
  ...
  private attachSharedMessageHandlerTo(wsAny: any) {
    try {
      ...
      if (typeof wsAny.addEventListener === "function") {
        try {
          const ls = wsAny.__listeners as
            | Map<string, Set<Function>>
            | undefined;
          const msgSet =
            ls && ls.get && typeof ls.get === "function"
              ? ls.get("message")
              : undefined;
          const hasHandler = msgSet
            ? Array.from(msgSet).some((f: any) => f === this.handleMessage)
            : false;
          if (!hasHandler)
            wsAny.addEventListener("message", this.handleMessage);
          try {
            (wsAny as any).__sharedMessageHandlerAttached = true;
          } catch (_) {}
        } catch (_) {
          try {
            wsAny.addEventListener("message", this.handleMessage);
  ...
  constructor(opts: RealtimeConnectionOptions = {}) {
    ...
    try {
      this._connectionController = new (
  ...
  async connect(token?: string): Promise<void> {
    ...
    const OPEN_STATE =
      (global as any).WebSocket && (global as any).WebSocket.OPEN !== undefined
        ? (global as any).WebSocket.OPEN
        : 1;
    if (
      this.ws?.readyState === OPEN_STATE ||
    ...
    return new Promise((resolve, reject) => {
      ...
      const atomicallyReject = (
        err?: any,
        expectedReject?: (e?: any) => void,
      ) =>
        this._connectionController &&
        typeof this._connectionController.atomicallyReject === "function"
          ? this._connectionController.atomicallyReject(err, expectedReject, {
              mySeq,
              myDisconnectMark,
              connectToken,
              myWsInstance,
            })
          : this._atomicallyReject(err, expectedReject, {
              mySeq,
              myDisconnectMark,
              connectToken,
              myWsInstance,
      ...
      console.log(
        "[RealtimeManager.connect] start url=",
        this.options.url,
        "timeout=",
        this.options.timeout,
      );
      // Use the instance factory helper to create the socket instance.
      const createWebSocketInstance = (tokenArg?: string) =>
        this._createWebSocketInstance(tokenArg);
      ...
      try {
        ...
        const forwardError = (ev: any) => {
          try {
            __earlyError = ev;
          } catch (_) {}
        };
        const forwardClose = (ev: any) => {
          try {
            __earlyClose = ev;
          } catch (_) {}
        };
        if (wsAny2) {
          try {
            if (typeof wsAny2.addEventListener === "function") {
              ...
              try {
                (wsAny2 as any).__earlyForwarders = {
                  forwardError,
                  forwardClose,
                };
              } catch (_) {}
              ...
              try {
                ...
                wsAny2.onerror = (e: any) => {
                  try {
                    forwardError(e);
                  } catch (_) {}
                  try {
                    if (typeof __prevErrHandler === "function")
                      __prevErrHandler.call(wsAny2, e);
                  } catch (_) {}
                };
              } catch (_) {}
              try {
                __prevCloseHandler = wsAny2.onclose;
                wsAny2.onclose = (e: any) => {
                  try {
                    forwardClose(e);
                  } catch (_) {}
                  try {
                    if (typeof __prevCloseHandler === "function")
                      __prevCloseHandler.call(wsAny2, e);
                  } catch (_) {}
                };
              } catch (_) {}
              try {
                (wsAny2 as any).__earlyPrevHandlers = {
                  prevErr: __prevErrHandler,
                  prevClose: __prevCloseHandler,
                };
              } catch (_) {}
            }
          } catch (_) {}
      ...
      const timeout = setTimeout(() => {
        try {
          if (
            this._connectionController &&
            typeof this._connectionController.handleConnectTimeout ===
              "function"
          ) {
            this._connectionController.handleConnectTimeout(
              mySeq,
              myDisconnectMark,
              connectToken,
              myAttemptCtrl,
      ...
      try {
        ...
        try {
          console.debug("[RealtimeManager.connect] set _connectTimeout", {
            mySeq,
            timeoutExists: !!this._connectTimeout,
          });
        } catch (_) {}
      } catch (_) {
        this._connectTimeout = undefined;
      ...
      const onOpen = () => {
        if (
          this._connectionController &&
          typeof this._connectionController.handleConnectOpen === "function"
        ) {
          return this._connectionController.handleConnectOpen({
            mySeq,
            myAttemptCtrl,
            timeout,
        ...
        return this._handleConnectOpen({
          mySeq,
          myAttemptCtrl,
          timeout,
          setSettled: (v: boolean) => {
      ...
      const onClose = (ev?: any) => {
        if (
          this._connectionController &&
          typeof this._connectionController.handleConnectClose === "function"
        ) {
          return this._connectionController.handleConnectClose({
            ev,
            mySeq,
            timeout,
            getSettled: () => settled,
        ...
        return this._handleConnectClose({
          ev,
          mySeq,
          timeout,
          getSettled: () => settled,
      ...
      const onError = (e?: any) => {
        if (
          this._connectionController &&
          typeof this._connectionController.handleConnectError === "function"
        ) {
          return this._connectionController.handleConnectError({
            e,
            mySeq,
            timeout,
            atomicallyReject,
        ...
        return this._handleConnectError({
          e,
          mySeq,
          timeout,
          atomicallyReject,
          ...
          try {
            ...
            if (!(this.ws as any).__sharedMessageHandlerAttached) {
              ...
              (this.ws as any).onmessage = (ev: any) => {
                ...
                try {
                  if (typeof prevMessage === "function")
                    prevMessage.call(this.ws, ev);
                } catch (_) {}
              };
            }
          } catch (_) {
            try {
              (this.ws as any).onmessage = onMessage;
            } catch (_) {}
          }
            ...
            (this.ws as any).onclose = (ev: any) => {
              ...
              try {
                if (typeof prevClose === "function")
                  prevClose.call(this.ws, ev);
              } catch (_) {}
            };
          } catch (_) {
            try {
              (this.ws as any).onclose = onClose;
            } catch (_) {}
          }
            ...
            (this.ws as any).onerror = (ev: any) => {
              ...
              try {
                if (typeof prevError === "function")
                  prevError.call(this.ws, ev);
              } catch (_) {}
            };
          } catch (_) {
            try {
              (this.ws as any).onerror = onError;
            } catch (_) {}
          }
        }
      } catch (err) {}
      ...
      try {
        ...
        if (wsAny2) {
          try {
            if (
              typeof wsAny2.removeEventListener === "function" &&
              (wsAny2 as any).__earlyForwarders
            ) {
              try {
                wsAny2.removeEventListener(
                  "error",
                  (wsAny2 as any).__earlyForwarders.forwardError,
                );
              } catch (_) {}
              try {
                wsAny2.removeEventListener(
                  "close",
                  (wsAny2 as any).__earlyForwarders.forwardClose,
                );
              } catch (_) {}
              try {
                delete (wsAny2 as any).__earlyForwarders;
              } catch (_) {}
              ...
              const prev = (wsAny2 as any).__earlyPrevHandlers as {
                prevErr?: any;
                prevClose?: any;
              };
              try {
                ...
                wsAny2.onerror = (ev: any) => {
                  try {
                    if (typeof existingOnError === "function")
                      existingOnError.call(wsAny2, ev);
                  } catch (_) {}
                  try {
                    if (typeof prev.prevErr === "function")
                      prev.prevErr.call(wsAny2, ev);
                  } catch (_) {}
                };
              } catch (_) {}
              try {
                ...
                wsAny2.onclose = (ev: any) => {
                  try {
                    if (typeof existingOnClose === "function")
                      existingOnClose.call(wsAny2, ev);
                  } catch (_) {}
                  try {
                    if (typeof prev.prevClose === "function")
                      prev.prevClose.call(wsAny2, ev);
                  } catch (_) {}
                };
              } catch (_) {}
              try {
                delete (wsAny2 as any).__earlyPrevHandlers;
              } catch (_) {}
            }
          } catch (_) {}
        }
  ...
  private dispatch(msg: any) {
    ...
    try {
      ...
      if (msg.type === "stream_complete" || msg.type === "stream_error") {
        try {
          const req =
            msg && msg.requestId
              ? this.streamingRequests.get(msg.requestId)
              : undefined;
          if (req) {
    ...
    try {
      const t =
        msg && msg.type ? msg.type.toString().replace(/-/g, "_") : undefined;
    ...
    for (const s of Array.from(this.subscriptions.values())) {
      try {
        ...
        if (s.event === "*") {
          ...
          try {
            console.debug(
              "[RealtimeManager.dispatch] wildcard match, invoking",
              {
                subId: s.id,
                event: s.event,
                payloadSummary:
          ...
          try {
            console.debug(
              "[RealtimeManager.dispatch] invoking callback start",
              { subId: s.id },
            );
            s.callback(payload);
            console.debug("[RealtimeManager.dispatch] invoking callback end", {
              subId: s.id,
            });
          } catch (err) {
            try {
        ...
        if (msg.type === "event" && msg.event && s.event === msg.event) {
          ...
          handledSubs.add(s.id);
          try {
            console.debug("[RealtimeManager.dispatch] direct event match", {
              subId: s.id,
              event: s.event,
            });
          } catch (_) {}
        ...
        try {
          ...
          const msgNorm = (normalizedType || msg.type || "")
            .toString()
            .replace(/-/g, "_");
          const matched = subNorm === msgNorm || s.event === originalType;
          try {
            console.debug("[RealtimeManager.dispatch] match check", {
              subId: s.id,
              subEvent: s.event,
              subNorm,
          ...
          if (matched) {
            try {
              console.debug(
                "[RealtimeManager.dispatch] matched invoking callback",
                { subId: s.id, subEvent: s.event },
              );
            } catch (_) {}
            ...
            const payload =
              msg && msg.type === "event" ? msg.data : msg && (msg.data || msg);
            if (handledSubs.has(s.id)) continue;
            handledSubs.add(s.id);
            try {
              console.debug(
              ...
              s.callback(payload);
              console.debug(
                "[RealtimeManager.dispatch] invoking callback end",
                { subId: s.id },
              );
            } catch (err) {
              try {
          ...
          try {
            console.debug("[RealtimeManager.dispatch] matching error", {
              subId: s.id,
              err: (err as any) && (err as any).message,
            });
          } catch (_) {}
        }
      } catch (_) {}
  ...
  sendMessage(payload: any) {
    try {
      ...
      try {
        console.debug(
          "[RealtimeManager.sendMessage] ws===__lastMockWebSocket?",
          this.ws === (global as any).__lastMockWebSocket,
          "ws.url=",
          this.ws?.url,
          "payloadType=",
  ...
  startStreaming(opts: {
    ...
    const subId = opts.callback
      ? this.subscribe(eventName, opts.callback)
      : undefined;
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      try {
        if (
          this._currentConnectAbort &&
          typeof this._currentConnectAbort.abort === "function"
        ) {
          try {
      ...
      try {
        console.debug(
          "[RealtimeManager.disconnect] called: _connectSeq=",
          this._connectSeq,
          "_connectToken=",
          this._connectToken,
        );
      ...
      try {
        if (this.ws) {
          ...
          try {
            if (typeof this.ws.close === "function") this.ws.close(1000);
          } catch (_) {}
          try {
            this.ws = null;
          } catch (_) {
  ...
  private _clearConnectHandlers() {
    try {
      console.debug(
        "[RealtimeManager._clearConnectHandlers] clearing handlers/timers",
        {
          seq: this._connectSeq,
          token: this._connectToken,
          hasTimeout: !!this._connectTimeout,
          hasDeferred: !!this._connectDeferredTimeout,
          hasResolve: !!this._connectResolve,
          hasReject: !!this._connectReject,
        },
      );
    } catch (_) {}
    try {
    ...
    try {
      if (this._connectTimeout) {
        ...
        try {
          console.debug(
            "[RealtimeManager._clearConnectHandlers] cleared _connectTimeout",
          );
        } catch (_) {}
      }
    } catch (_) {}
    try {
      if (this._connectDeferredTimeout) {
        ...
        try {
          console.debug(
            "[RealtimeManager._clearConnectHandlers] cleared _connectDeferredTimeout",
          );
        } catch (_) {}
      }
    } catch (_) {}
    ...
    try {
      this._connectResolve = () => {
        try {
          console.debug(
            "[RealtimeManager._clearConnectHandlers] swallowed late resolve",
          );
        } catch (_) {}
      };
    } catch (_) {}
    try {
      this._connectReject = (_e?: any) => {
        try {
          console.debug(
            "[RealtimeManager._clearConnectHandlers] swallowed late reject",
            _e,
          );
        } catch (_) {}
      };
  ...
  private _runDeferredConnectReject(
    ...
    try {
      ...
      try {
        if (
          myAttemptCtrl &&
          myAttemptCtrl.signal &&
          myAttemptCtrl.signal.aborted
        ) {
          return;
        }
      } catch (_) {}
      ...
      try {
        ...
        try {
          ...
          if (
            currentReady2 === OPEN2 ||
            currentReady2 === (WebSocket as any).OPEN
          )
            return;
        } catch (_) {}
...
export const createRealtimeManager = (opts?: any): RealtimeManager =>
  new RealtimeManager(opts);
