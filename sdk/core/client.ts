...
export class SchillingerSDK {
    ...
    try {
      ...
      return {
        ...
        collaborationSupport: Boolean(
          capabilities.collaborationSupport ?? this.config.enableCollaboration,
        ),
        ...
        limits: {
          ...
          maxConcurrentRequests:
            capabilities.limits?.maxConcurrentRequests ||
      ...
      if (this.config.environment === "production") {
        throw new Error(
          `Server capabilities unavailable: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
      ...
      if (this.config.debug) {
        console.warn(
          "[Schillinger SDK] Using fallback server capabilities due to error:",
          error,
        );
      }
    ...
    try {
      ...
      const quotaData = {
        ...
        remaining: {
          ...
          monthly:
            Number(data.remaining?.monthly) || limits.monthly - usage.monthly,
          storage:
            Number(data.remaining?.storage) || limits.storage - usage.storage,
          ...data.remaining,
        },
        ...data,
      };
      ...
      if (this.config.environment === "production") {
        throw new Error(
          `Quota information unavailable: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
      ...
      if (this.config.debug) {
        console.warn(
          "[Schillilling SDK] Using fallback quota information due to error:",
          error,
        );
      }
      ...
      const fallbackQuota = {
        ...
        remaining: {
          ...
          monthly: Math.max(
            0,
            10000 - (Number(mockUsage.monthlyRequests) || 0),
          ),
          storage: Math.max(
            0,
            1073741824 - (Number(mockUsage.storageUsed) || 0),
          ),
        },
  ...
  async connectRealtime(): Promise<void> {
    ...
    const tokenInfo = (this.authManager as any).getTokenInfo
      ...
      : undefined;
    console.log(
      "[SchillingerSDK.connectRealtime] calling realtimeManager.connect",
    );
    await this.realtimeManager.connect(tokenInfo?.token);
    console.log(
      "[SchillingerSDK.connectRealtime] returned from realtimeManager.connect, connectionState=",
      this.realtimeManager?.getConnectionState(),
    );
  }
  ...
  isRealtimeConnected(): boolean {
    try {
      ...
      console.log(
        "[SchillingerSDK.isRealtimeConnected] realtimeManager=",
      ...
      console.log(
        "[SchillingerSDK.isRealtimeConnected] error reading state",
        e,
      );
      return false;
    }
